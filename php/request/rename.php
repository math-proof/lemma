...
function rename_lemma($old, $new)
{
    if (std\is_linux()) {
        ...
        $args = "-src " . std\escapeshellarg($old) . " -dst ". std\escapeshellarg($new);
        $cmd = "powershell -ExecutionPolicy Bypass -NoProfile -File ".  std\escapeshellarg("ps1\\rename.ps1") . " -ArgumentList $args";;
    }
