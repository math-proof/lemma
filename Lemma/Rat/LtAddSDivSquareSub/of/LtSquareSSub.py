...
def prove(Eq):
    ...
    Eq << Eq[-1].this.find(Mul).apply(Nat.Mul.eq.Square)

    Eq.lt = Eq[-1].this.rhs.apply(Nat.Mul.eq.Square)
    ...
    Eq << Nat.Mul.eq.Add.Square.apply(Eq[-1].rhs.find(Mul) / 2)
    ...
    Eq << Eq[-1].this.rhs.args[0].apply(Int.Square.Neg)
