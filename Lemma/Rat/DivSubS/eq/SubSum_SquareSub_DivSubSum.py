...
@prove
def prove(Eq):
    from Lemma import Algebra, Bool, Finset, Int, Nat, Rat
    ...
    y = Symbol(Stack[i:n - 1](Piecewise((x[i], i < t),(x[i + 1], True))))
    Eq << y[i].this.definition

    Eq.y_sum = Finset.EqSum.of.Eq_Ite.apply(Eq[1], Sum[i:n-1](y[i]))

    Eq << Rat.Sum.Square.eq.Div.Sum.Square.apply(Sum[i:n-1]((y[i] - Sum[i:n - 1](y[i]) / (n - 1)) ** 2))

    Eq << Eq[-1].subs(Eq.y_sum).reversed

    Eq << Finset.EqSum.of.Eq_Ite.apply(Eq[1], Eq[-1].rhs)

    Eq << Bool.Eq.of.Eq.Eq.apply(Eq[-2], Eq[-1])

    Eq << Finset.Sum_SquareSub.eq.Sub_SquareSum.apply(Eq[-1].lhs.args[1])

    Eq << Eq[-1].subs(Eq.y_sum)

    Eq << Finset.EqSum.of.Eq_Ite.apply(Eq[1], Sum[i:n - 1](y[i] ** 2))
    ...
    Eq << Eq[0].lhs.find(Sum).this.apply(Finset.Sum_SquareSub.eq.Sub_SquareSum)
    ...
    Eq << Eq[-1].this.apply(Int.EqAdd.Is.Eq_Sub, lhs=slice(0, None, 2))
