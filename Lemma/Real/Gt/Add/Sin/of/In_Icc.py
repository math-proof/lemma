...
def prove(Eq):
    ...
    Eq << Eq[-1].this.find(csc).apply(Real.Csc.eq.Inv.Sin)

    Eq << Eq[-1].this.find(cos).apply(Real.Cos.eq.Sub.double)

    Eq.eq_grad = Eq[-1].this.find(cot).apply(Real.Cot.eq.Div)
    ...
    Eq << sin(x).this.apply(Real.Sin.eq.Mul.double)
