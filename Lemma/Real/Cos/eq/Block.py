...
def prove(Eq):
    ...
    Eq << Eq[-1].this.lhs.apply(Real.Cos.eq.Ite)

    Eq << Eq[-1].this.find(Cos[Piecewise]).apply(Real.Cos.eq.Ite)

    Eq << Eq[-1].this.find(Cos[Piecewise]).apply(Real.Cos.eq.Ite, simplify=0)
