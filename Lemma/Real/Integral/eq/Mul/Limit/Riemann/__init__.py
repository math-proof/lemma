...
def prove(Eq):
    ...
    Eq << (a < b).this.apply(Real.Integral.eq.Mul.Limit.Maxima.Darboux.of.Lt, Eq[0].lhs)

    Eq << (a < b).this.apply(Real.Integral.eq.Mul.Limit.Minima.Darboux.of.Lt, Eq[0].lhs)
    ...
    Eq << Eq[-1].this.rhs.apply(Real.EqDarboux.of.Lt.Eq_Limit)

    Eq << Bool.Cond.given.Imp.ImpNot.apply(Eq[2], cond=a > b)

    Eq <<= Eq[-2].this.apply(Bool.Imp.flatten), Eq[-1].this.apply(Bool.Imp.flatten)

    Eq << Eq[-2].this.lhs.apply(Real.Integral.eq.Limit.Riemann.of.Gt, Eq[0].lhs)
