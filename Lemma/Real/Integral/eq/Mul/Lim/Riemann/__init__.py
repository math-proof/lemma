...
@prove
def prove(Eq):
    from Lemma import Real, Bool, Rat, Nat
    ...
    Eq << (a < b).this.apply(Real.Integral.eq.Mul.Lim.Maxima.Darboux.of.Lt, Eq[0].lhs)

    Eq << (a < b).this.apply(Real.Integral.eq.Mul.Lim.Minima.Darboux.of.Lt, Eq[0].lhs)
    ...
    Eq << Eq[-1].this.rhs.args[1].apply(Nat.Gt_0.of.Lt)

    Eq << Eq[-1].this.rhs.apply(Rat.EqDiv.of.Gt_0.Eq)

    Eq << Bool.Imp_And.of.ImpAnd.apply(Eq[-1])

    Eq << Eq[-1].this.rhs.apply(Real.EqDarboux.of.Lt.Eq_Lim)
