...
def prove(Eq):
    ...
    Eq << Eq.one.this.lhs.apply(Real.Integral.Sin.eq.Mul.gamma.wallis)

    Eq.induct = Eq[0].subs(m, m + 2)

    Eq << Eq.induct.this.lhs.expr.expand()

    Eq << Eq[-1].this.lhs.apply(Real.Integral.eq.Add.by_parts)
    ...
    Eq << Eq.two.this.lhs.apply(Real.Integral.limits.subst, sin(x), x)
    Eq << Eq[-1].this.lhs.apply(Real.Integral.Pow.eq.Mul)
    Eq << Eq[-1].this.rhs.expand(func=True)
