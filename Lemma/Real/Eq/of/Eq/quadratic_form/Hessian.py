...
def prove(Eq):
    ...
    Eq << Eq[-1].this.rhs.args[0].apply(Real.Grad_Add_Dot.simp)

    Eq << Eq[-1].this.rhs.args[1].apply(Real.Grad_DotDot.eq.DotAdd)

    Eq << Real.EqGrad.of.Eq.apply(Eq[-1], (x,))

    Eq << Eq[-1].this.rhs.apply(Real.Grad.eq.Add)

    Eq << Eq[-1].this.rhs.apply(Real.Grad_Add_Dot.simp)
