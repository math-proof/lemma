...
def prove(Eq):
    ...
    Eq << Set.Eq.of.In.definition.apply(Eq[0], 'y')

    y = Eq[-1].lhs
    δ_0 = Symbol(positive=True)
    Eq <<= Real.Any.All.of.Eq_Limit.limit_definition.apply(Eq[-1], delta=δ_0), Eq[1].subs(Eq[-1].reversed)

    δ = Symbol(positive=True)
    Eq << Real.Eq_Limit.given.Any_All.limit_definition.apply(Eq[-1], delta=δ)

    Eq << Greater(y, 0, plausible=True)

    δ_1 = Symbol(positive=True)
    Eq << Real.Any.All.Gt.of.Gt_0.Eq_Limit.apply(Eq[-1], Eq[2], delta=δ_1)
