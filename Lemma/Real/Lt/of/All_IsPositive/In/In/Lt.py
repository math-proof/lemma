...
def prove(Eq):
    ...
    Eq.subset = Set.Subset.Icc.of.In.In.apply(Eq[1], Eq[2])

    Eq.is_positive_real = Set.All.of.Subset.All.apply(Eq.subset, Eq[0])

    Eq << Real.IsContinuous.of.IsDifferentiable.apply(Eq.is_positive_real)

    Eq << Nat.Le.of.Lt.apply(Eq[3])

    Eq.any = Real.Any.Eq.of.Le.IsContinuous.IsDifferentiable.mean_value_theorem.Lagrange.close.apply(Eq[-1], Eq[-2], Eq.is_positive_real)

    Eq << Eq.is_positive_real.this.expr.apply(Set.Gt_0.of.IsPositive)

    Eq << Nat.Gt_0.of.Lt.apply(Eq[3])

    Eq << Bool.All_And.of.All.Cond.apply(Eq[-1], Eq[-2], simplify=None)
