...
def prove(Eq):
    ...
    Eq << Eq[-1].this.lhs.apply(Real.Sin.eq.Ite)

    Eq << Eq[-1].this.find(Sin[Piecewise]).apply(Real.Sin.eq.Ite)

    Eq << Eq[-1].this.find(Sin[Piecewise]).apply(Real.Sin.eq.Ite, simplify=0)
