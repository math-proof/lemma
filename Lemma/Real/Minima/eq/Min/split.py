...
def prove(Eq):
    ...
    Eq << Eq[-1].this.find(Minima).apply(Real.Minima.Ite)

    Eq << Eq[-1].this.rhs.find(Minima).apply(Real.Minima.Ite)

    Eq << Eq[-1].this.rhs.find(Minima).apply(Real.Minima.Ite)

    Eq << Eq[-1].this.rhs.apply(Real.Min.eq.Minima)

    Eq << Eq[-1].this.find(Element).apply(Set.In.Is.In_Inter.ou.In_SDiff, B, simplify=None)

    Eq << Eq[-1].this.find(Piecewise).apply(Nat.Ite.eq.Min)
