...
def prove(Eq):
    ...
    Eq <<= Real.All_EqLimit.IsContinuous.Icc.apply(f(x), (x, domain)), Real.All_EqLimit.IsContinuous.Icc.apply(g(x), (x, domain))

    Eq <<= Eq[-1] & Eq[-2]

    Eq << Eq[-1].this.expr.apply(Real.Eq_Limit.Sub.of.Eq_Limit.Eq_Limit)
    ...
    Eq <<= Eq[-1].this.lhs.apply(Set.In_Icc.given.InAdd, b)
    ...
    Eq << Real.Limit.ge.Zero.of.Gt_0.All_GtLimit__0.apply(Eq[-1], Eq[-2])

    Eq << Real.EqSub.of.IsLimited.IsLimited.algebraic_limit_theorem.apply(Eq[2], Eq[3])

    Eq << Eq[-2].subs(Eq[-1])

    Eq << Set.GeAdd.of.Ge.In.apply(Eq[-1], Eq[3])
