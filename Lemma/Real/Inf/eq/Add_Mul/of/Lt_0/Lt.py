...
@prove
def prove(Eq):
    from Lemma import Nat, Rat, Real

    m, M, a, b = Symbol(real=True, given=True)
    x = Symbol(real=True)
    Eq << apply(a < 0, m < M, a * x + b, x)

    Eq << Eq[-1].this.lhs.apply(Real.Inf.eq.Add)

    Eq << Rat.Div.lt.Zero.of.Lt_0.apply(Eq[0])

    Eq << Real.Inf.eq.Mul.Sup.of.Lt_0.apply(Eq[0], Eq[-2].lhs)

    Eq << Eq[-3].subs(Eq[-1])

    Eq << Nat.Div.given.Eq.apply(Eq[-1], a)

    Eq << Real.EqSup.of.Lt.apply(Eq[1], x)
