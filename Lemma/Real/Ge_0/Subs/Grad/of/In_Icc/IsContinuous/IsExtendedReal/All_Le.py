from util import *


@apply
def apply(el_Interval, is_continuous, is_extended_real, all_le):
    from Lemma.Real.Any.Eq.Rolle.of.Lt.IsContinuous.IsDifferentiable.Eq import of_continuous
    fx, limit = of_continuous(is_continuous)
    x, a, b = limit

    from Lemma.Real.Any.Le.Rolle.of.Lt.IsContinuous.IsExtendedReal.IsExtendedReal.Eq import is_differentiable_at
    S[fx], S[limit] = is_differentiable_at(is_extended_real, -1)

    c, S[Interval.open(a, b)] = el_Interval.of(Element)
    (S[fx], S[fx._subs(x, c)]), S[limit] = all_le.of(All[LessEqual])
    return Subs[x:c](Derivative[x - S.Infinitesimal](fx)) >= 0


@prove
def prove(Eq):
    from Lemma import Real, Set, Bool, Int, Nat

    a, b, x, c = Symbol(real=True)
    f = Function(shape=(), real=True)
    from Lemma.Real.All.Any.Eq.of.All_Eq.intermediate_value_theorem import is_continuous
    Eq << apply(
        Element(c, Interval.open(a, b)),
        is_continuous(f, a, b),
        All[x:a:b](Element(Derivative[x - S.Infinitesimal](f(x)), ExtendedReals)),
        All[x:a:b](f(x) <= f(c)))

    Eq <<= Eq[-1].this.find(Derivative).apply(Real.Grad.eq.Lim.one_sided), Int.AllIco.of.AllIco.offset.apply(Eq[-2], c)

    Eq << Bool.Imp.of.AllSetOf.apply(Eq[-1])

    Eq << Int.InSub.of.In_Icc.apply(Eq[0], c)

    Eq.lt_zero = Set.Gt.of.In_Icc.apply(Eq[-1]).this.apply(Nat.Lt_0.of.Gt)

    Eq << Bool.ImpAnd.of.Imp.cond.apply(Eq[-1], Eq[-2])

    Eq << Eq[-1].this.lhs.apply(Set.In.In.given.And.In, stop=True)

    Eq << Bool.BFn.of.BFnIte.Cond.apply(Eq[-3], Eq[-1])

    Eq << Eq[-1].this.rhs.apply(Nat.Le_0.of.Le)

    Eq << Bool.Imp.And.of.Imp.apply(Eq[-1])

    Eq << Eq[-1].this.rhs.apply(Set.GeDiv.of.IsNegative.Le)

    Eq << Bool.All.of.All_Imp.apply(Eq[-1])

    Eq << Set.IsNegative.of.Lt_0.apply(Eq.lt_zero, simplify=None)

    δ = Symbol(real=True, negative=True)
    Eq << Set.Any_Eq.of.In.apply(Eq[-1], var=δ)

    Eq << Bool.Any_And.of.Any.All.apply(Eq[-3], Eq[-1], simplify=None)

    Eq.Any_All = Eq[-1].this.expr.apply(Bool.UFn.of.UFn.Eq)

    Eq << Bool.Imp.of.AllSetOf.apply(Eq[2])

    Eq << Eq[-1].subs(x, c)

    Eq << Bool.Cond.of.Imp.Cond.apply(Eq[0], Eq[-1])

    Eq << Eq[-1].this.find(Derivative).apply(Real.Grad.eq.Lim.one_sided, simplify=2)

    Eq << Eq.Any_All.this.find(All).limits_subs(x, Eq[-1].lhs.variable)

    Eq << Bool.Any_And.of.Any.All.apply(Eq[-1], Eq.Any_All, simplify=None)


    Eq << Eq[-1].this.expr.apply(Real.Ge_0.Lim.of.IsExtendedReal.All_Ge.one_sided)



if __name__ == '__main__':
    run()
# created on 2023-10-15
