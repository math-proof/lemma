...
def prove(Eq):
    ...
    y = Symbol(real=True)
    Eq <<= Real.GeSup.given.All_Any_Gt.apply(Eq[-1], y), Real.LeSup.given.All.Le.apply(Eq[-2])
    ...
    Eq <<= Eq[-2].this.expr.apply(Bool.Any_UFn.given.UFnUFn, x, (m + M) / 2), Bool.All_And.given.All.All.apply(Eq[-1])
    ...
    Eq <<= Eq[-3].this.lhs.apply(Nat.Lt.of.Lt.Lt, ret=1), Eq[-1].this.rhs.apply(Set.In.given.And.strengthen, upper=m, strict=True)

    Eq <<= Eq[-2].this.lhs.apply(Nat.LtAdd.of.Lt.Lt), Bool.Imp.given.Cond.apply(Eq[-1])
