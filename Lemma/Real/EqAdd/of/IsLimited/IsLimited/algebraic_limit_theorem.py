...
def prove(Eq):
    ...
    Eq << Real.Any.All.of.IsLimited.limit_definition.symbol_subs.apply(Eq[0], ε_0, δ_0, var='A')

    Eq << Eq[-1].subs(ε_0, ε / 2)

    ε_1 = Symbol(real=True, positive=True)
    δ_1 = Symbol(real=True, positive=True)
    Eq << Real.Any.All.of.IsLimited.limit_definition.symbol_subs.apply(Eq[1], ε_1, δ_1, var='B')
    ...
    Eq << Eq[-1].this.find(And, Less).apply(Set.Lt.given.In.Icc)
