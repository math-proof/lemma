...
@prove
def prove(Eq):
    from Lemma import Set, Real, Nat, Int, Rat
    ...
    Eq.le = Eq[-1].this.rhs.apply(Real.Lim.eq.Mul)

    Eq << Real.Eq_0.Lim.of.In_Icc.geometric_series.positive.apply(Eq[0], n)

    Eq << Set.IsReal.of.Eq_0.apply(Eq[-1])

    Eq << Real.Eq.Abs.Lim.of.IsLimited.apply(Eq[-1])
