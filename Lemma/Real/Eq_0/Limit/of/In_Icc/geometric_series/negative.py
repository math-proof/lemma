...
@prove
def prove(Eq):
    from Lemma import Set, Real, Nat, Int
    ...
    Eq.is_zero = Real.Eq_0.Limit.of.In_Icc.geometric_series.positive.apply(Eq[-1], n)

    Eq <<= Int.LeNegAbs.apply(Eq[1].find(Pow)), Int.GeAbs.apply(Eq[1].find(Pow))

    Eq <<= Real.GeLimit.of.Ge.apply(Eq[-2], (n, oo)), Real.LeLimit.of.Le.apply(Eq[-1], (n, oo))

    Eq << Eq[-2].this.rhs.apply(Real.Limit.eq.Mul)
