...
@prove
def prove(Eq):
    from Lemma import Real, Algebra, Set, Bool, Nat, Int, Rat

    n = Symbol(integer=True, positive=True)
    x = Symbol(real=True, given=True)
    Eq << apply(Element(x, Interval(0, 1, left_open=True, right_open=True)), n)

    Eq << Real.Eq_Limit.given.Any_All.limit_definition.restricted.apply(Eq[1])
    ...
    Eq.lt_zero = Set.Lt_0.Log.of.In_Icc.apply(Eq[0])
