...
def prove(Eq):
    ...
    Eq << Eq[-1].this.lhs.apply(Real.Tan.eq.Ite)

    Eq << Eq[-1].this.find(Tan[Piecewise]).apply(Real.Tan.eq.Ite)

    Eq << Eq[-1].this.find(Tan[Piecewise]).apply(Real.Tan.eq.Ite, simplify=0)
