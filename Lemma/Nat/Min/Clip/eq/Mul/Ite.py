...
@prove
def prove(Eq):
    from Lemma import Bool, Int, Nat, Rat
    ...
    Eq <<= Eq[-2].this.rhs.apply(Rat.Lt_0.Eq.given.And.Div), Eq[-1].this.rhs.apply(Rat.Gt_0.Eq.given.And.Div)
    ...
    Eq <<= Eq[-2].this.find(Min).apply(Nat.Min.eq.Mul), Eq[-1].this.find(Min).apply(Nat.Min.eq.Mul.Max)
    ...
    Eq << Eq[-1].this.find(Max).apply(Nat.Max.eq.Ite.Gt)
