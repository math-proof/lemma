...
def prove(Eq):
    ...
    i = Eq[-1].rhs.variable
    Eq << Eq[-1].this.lhs.args[0].apply(Finset.ProdIco.eq.Prod_UFnSub, i, n - i)

    Eq << Eq[-1].this.find((~Product) ** -1).apply(Finset.ProdIco.eq.Prod_UFnSub, i, n - i)

    Eq << Eq[-1].this.lhs.apply(Finset.DivProdS.eq.ProdSDiff)
