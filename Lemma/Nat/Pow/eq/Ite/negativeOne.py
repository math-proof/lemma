...
def prove(Eq):
    ...
    Eq << Eq[1].this.find(Equal & ~Equal).apply(Nat.Eq_even.Is.Eq)

    Eq << Eq[-1].this.find(Unequal).apply(Nat.Ne_0.Is.Eq_odd)

    Eq << Eq[-1].this.find(Equal & ~Equal).apply(Nat.Eq_odd.Is.Eq)
