...
-- proof
  ...
  have h_i : i < ((s.eraseIdx d).insertIdx d 1)[0]'(by grind) := by
    ...
    ·
      ...
      omega
  rw [GetRepeat.eq.Cast_RepeatGet.of.Lt_Get_0.GtVal_0.fin h_d h_i] at h
  have h_i : i < (s.eraseIdx d)[0]'(by grind) := by
    rwa [h_get_eraseIdx]
  simp [GetUnsqueeze.eq.Cast_UnsqueezeGet.of.Lt_Get_0.Gt_0.GtLength_0.fin _ h_d h_i] at h
