...
-- proof
  ...
  have h_i : i < ((s.eraseIdx d).insertIdx d 1)[0]'(by grind) := by
    rw [GetInsertIdx.eq.Get.of.Gt.GeLength _ h_d]
    ·
      rwa [h_get_eraseIdx]
    ·
      rw [LengthEraseIdx.eq.SubLength_1.of.GtLength h_s]
      omega
  rw [GetRepeat.eq.Cast_RepeatGet.of.GtGet_0.GtVal_0.fin h_d h_i] at h
  have h_i : i < (s.eraseIdx d)[0]'(by grind) := by
    rwa [h_get_eraseIdx]
  simp [GetUnsqueeze.eq.Cast_UnsqueezeGet.of.GtGet_0.Gt_0.GtLength_0.fin _ h_d h_i] at h
  ...
  have h_lt_length : d - 1 < ((s.eraseIdx d).tail.insertIdx (d - 1) 1).length := by
    ...
    apply GtLengthInsertIdxEraseIdx.of.GtLength h_lt_length_tail
  ...
  ·
    ...
    rw [GetTail.eq.Get_Add_1.of.GtLengthTail h_lt_length_tail]
