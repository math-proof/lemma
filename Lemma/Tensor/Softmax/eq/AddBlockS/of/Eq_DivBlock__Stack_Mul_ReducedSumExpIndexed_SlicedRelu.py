...
def prove(Eq):
    ...
    Eq << Eq[-1].this(i, j).find(And).apply(Set.OrLt.Or.Is.In.Range.BandPart.lower)

    Eq << Eq[-1].this.find(And).apply(Algebra.And.collect, cond=Eq[-1].rhs.args[1].find(Element))

    Eq.zij_dquote_def = Eq[-1].this(i, j).find(And).apply(Set.Or.Or.Is.In.Range.BandPart.upper.Min)
