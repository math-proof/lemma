...
def apply(eq):
    ...
    eq.of(Equal[
        BlockMatrix[
            ...
            Stack[Exp, Tuple[Expr - Expr]]] / Stack[Ones * ReducedSum[Exp]]])

    assert n >= 2 and l >= 2 and l <= n

    return Equal(softmax(A + (BandPart[l - 1, 0](Ones(n, n)) - 1) * oo), BlockMatrix(
        Stack[i:l](BlockMatrix(z[i, l - i - 1:], Zeros(n - 1 - i))),
        Stack[i:n - l](BlockMatrix(Zeros(i + 1), z[i + l], Zeros(n - 1 - i - l)))))
...
def prove(Eq):
    ...
    Eq << apply(Equal(z, BlockMatrix(
            Stack[i:l](BlockMatrix(Zeros(l - i - 1), Exp(A[i, :i + 1]))),
            Stack[i:n - l](Exp(A[i + l, i + 1:i + l + 1]))) / Stack[i:n](Ones(l) * ReducedSum(Exp(A[i, relu(i + 1 - l):i + 1])))))
    ...
    Eq << Eq[-1].this(i, j).find(And).apply(Set.Or.Or.Is.sub.In.Range.BandPart.lower)
