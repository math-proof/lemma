...
-- proof
  ...
  | nil =>
    ...
    | [] =>
      ...
      apply (SEqMapS.of.SEq hR f).trans
      refine (Bool.SEq.of.Eq (MapBmm.eq.BmmMapS.of.All_Eq_Add.All_Eq_Mul h_mul h_add
        (A : Tensor α ([] ++ [m, t]))
        (C : Tensor α ([] ++ [t, k])))).trans hL.symm
    | _ :: _ =>
      simp at hlen
  | cons n s ih =>
    match s' with
    ...
    | n' :: s' =>
      ...
      have hcastA : ((n :: s) ++ [m, t]).set 0 (n ⊔ n') = (n ⊔ n' :: s) ++ [m, t] := by
        rw [List.SetAppend.eq.Append_Set.of.GtLength (by simp)]
        simp [List.Set_0.eq.Cons_Tail.of.GtLength_0]
      have hcastC : ((n' :: s') ++ [t, k]).set 0 (n ⊔ n') = (n ⊔ n' :: s') ++ [t, k] := by
        rw [List.SetAppend.eq.Append_Set.of.GtLength (by simp)]
        simp [List.Set_0.eq.Cons_Tail.of.GtLength_0]
      let Ar : Tensor α ((n ⊔ n' :: s) ++ [m, t]) := cast (congrArg (Tensor α) hcastA) (A.resize ⟨0, by grind⟩ (n ⊔ n'))
      let Afr : Tensor β ((n ⊔ n' :: s) ++ [m, t]) := cast (congrArg (Tensor β) hcastA) ((F A).resize ⟨0, by grind⟩ (n ⊔ n'))
      let Cr : Tensor α ((n ⊔ n' :: s') ++ [t, k]) := cast (congrArg (Tensor α) hcastC) (C.resize ⟨0, by grind⟩ (n ⊔ n'))
      let Crf : Tensor β ((n ⊔ n' :: s') ++ [t, k]) := cast (congrArg (Tensor β) hcastC) ((F C).resize ⟨0, by grind⟩ (n ⊔ n'))
      ...
      have hL := Matmul.as.MatmulResizeS.of.Length.GtLength_0 (s := n :: s) (s' := n' :: s') (by simp) hlen (F A) (F C)
      have hR := Matmul.as.MatmulResizeS.of.Length.GtLength_0 (s := n :: s) (s' := n' :: s') (by simp) hlen A C
      have hmat : F (Ar.matmul Cr (by simpa using hlen')) = Afr.matmul Crf (by simpa using hlen') := by
        rw [hAfr, hCrf]
        have hshape : broadcast_shape (n ⊔ n' :: s) (n ⊔ n' :: s') ++ [m, k] = (n ⊔ n') :: (broadcast_shape s s' ++ [m, k]) := by
          simp [broadcast_shape]; split_ifs <;> simp_all
        let L : Tensor α ((n ⊔ n') :: (broadcast_shape s s' ++ [m, k])) := cast (congrArg (Tensor α) hshape) (Ar.matmul Cr (by simpa using hlen'))
        let R : Tensor β ((n ⊔ n') :: (broadcast_shape s s' ++ [m, k])) := cast (congrArg (Tensor β) hshape) ((F Ar).matmul (F Cr) (by simpa using hlen'))
        have hLR : F L = R := by
          ...
          have hlenA : (n ⊔ n' :: s).length = (n ⊔ n' :: s').length := by
            simpa using hlen'
          have hgetL := GetMatmul.as.MatmulGetS.of.Get_0.Length.GtLength_0
            (s := n ⊔ n' :: s) (s' := n ⊔ n' :: s')
            (by simp) hlenA (by rfl) Ar Cr i
          have hgetR := GetMatmul.as.MatmulGetS.of.Get_0.Length.GtLength_0
            (s := n ⊔ n' :: s) (s' := n ⊔ n' :: s')
            (by simp) hlenA (by rfl) (F Ar) (F Cr) i
          have hCL := GetCast.as.Get.of.Eq.GtLength_0.right.fin (by simp) hshape (Ar.matmul Cr (by simpa using hlen')) i
          have hCR := GetCast.as.Get.of.Eq.GtLength_0.right.fin (by simp) hshape ((F Ar).matmul (F Cr) (by simpa using hlen')) i
          ...
          exact (SEqMatmulS.of.SEq.SEq (by simpa using hlen')
              ((GetCast.as.Get.of.Eq.GtLength_0.right.fin (by simp) hXA (F Ar) i).trans (Bool.SEq.of.Eq hAi)).symm
              ((GetCast.as.Get.of.Eq.GtLength_0.right.fin (by simp) hYA (F Cr) i).trans (Bool.SEq.of.Eq hCi)).symm)
        apply Bool.Eq.of.SEq
