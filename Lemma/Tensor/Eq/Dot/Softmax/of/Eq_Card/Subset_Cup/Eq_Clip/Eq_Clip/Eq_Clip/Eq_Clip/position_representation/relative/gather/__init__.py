...
def apply(eq_cup, subset, eq_K_quote, eq_V_quote, eq_K_dquote, eq_V_dquote, Q, K, V):
    ...
    ((w_K, clip_index), j_limit, i_limit), K_dquote = eq_K_dquote.of(Equal[Stack[Indexed]])
    ((w_V, S[clip_index]), S[j_limit], S[i_limit]), V_dquote = eq_V_dquote.of(Equal[Stack[Indexed]])
...
def prove(Eq):
    ...
    Eq << apply(
        ...
        Equal(K_dquote, Stack[j:m, i:n](w_K[k + clip(d[j] - i, -k, k)])),
        Equal(V_dquote, Stack[j:m, i:n](w_V[k + clip(d[j] - i, -k, k)])),
        Q, K, V)
