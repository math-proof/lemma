...
def prove(Eq):
    ...
    Eq << Tensor.All.Eq.Slice.of.All_Eq.apply(Eq[-1], slice(i + 1 - Min(i + 1, l), i + 1))

    Eq << Eq[-1].this.rhs.find(~Indexed - Indexed).args[1].apply(Nat.Expr.eq.IteGe, upper=n - 1)

    Eq << Eq[-1].this(i).rhs(j).find(GreaterEqual).simplify()

    Eq.V_relu = Eq[-1].this.rhs.find(-~Min).apply(Tensor.Min.eq.Add.Relu, index=1)

    Eq << Eq[1][i]

    Eq << Bool.AllIn.of.All.apply(Eq[-1], (i, 0, n), simplify=None)

    Eq << Tensor.All.Eq.Slice.of.All_Eq.apply(Eq[-1], slice(0, Min(i + 1, l)))
