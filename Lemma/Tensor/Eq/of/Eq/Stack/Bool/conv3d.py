...
def apply(eq_M, x, w, r):
    (((i, (β0, ζ0)), (j, (β1, ζ1)), (t, (β2, ζ2))), (S[t], S[0], n2), (S[j], S[0], n1), (S[i], S[0], n0), (k, S[0], m)), M = eq_M.of(Equal[Stack[Bool[Element[Range] & Element[Range] & Element[Range]]]])
    ...
    M0 = Stack[h:d, t:n2, j:n1, i:n0, k:m](M[k, i, j, t])
    M1 = Stack[h:d_, t:n2, j:n1, i:n0, k:m](M[k, i, j, t])

    block = conv3d[r](x[k][β0[k]:ζ0[k], β1[k]:ζ1[k], β2[k]:ζ2[k]], w)

    block = BlockMatrix[2](Zeros(ζ0[k] - β0[k], ζ1[k] - β1[k], β2[k], d_), block, Zeros(ζ0[k] - β0[k], ζ1[k] - β1[k], n2 - ζ2[k], d_))

    block = BlockMatrix[1](Zeros(ζ0[k] - β0[k], β1[k], n2, d_), block, Zeros(ζ0[k] - β0[k], n1 - ζ1[k], n2, d_))

    block = BlockMatrix(Zeros(β0[k], n1, n2, d_), block, Zeros(n0 - ζ0[k], n1, n2, d_))
...
def prove(Eq):
    ...
    Eq.mul_floor_0 = d0.this.definition.reversed.this.apply(Int.EqAdd.Is.Eq_Sub, lhs=0)

    d1 = Symbol((l[1] - 1) // 2 * r[1] + (r[1] // 2) * (1 - l[1] % 2))
    Eq.mul_floor_1 = d1.this.definition.reversed.this.apply(Int.EqAdd.Is.Eq_Sub, lhs=0)

    d2 = Symbol((l[2] - 1) // 2 * r[2] + (r[2] // 2) * (1 - l[2] % 2))
    Eq.mul_floor_2 = d2.this.definition.reversed.this.apply(Int.EqAdd.Is.Eq_Sub, lhs=0)
    ...
    Eq << Eq[-1].this.find(Min[Ceil]).apply(Nat.Min.eq.Add, 1).this.find(Min[Ceil]).apply(Nat.Min.eq.Add, 1).this.find(Min[Ceil]).apply(Nat.Min.eq.Add, 1)
    ...
    Eq << Eq[-1].this.find(Min[Floor]).apply(Int.Min.eq.Floor).this.find(Min[Floor]).apply(Int.Min.eq.Floor).this.find(Min[Floor]).apply(Int.Min.eq.Floor)

    Eq << Eq[-1].this.find(-Floor).apply(Int.Mul.eq.Ceil).this.find(-Floor).apply(Int.Mul.eq.Ceil).this.find(-Floor).apply(Int.Mul.eq.Ceil)
    ...
    Eq << Eq[-1].this.find(-Add).apply(Nat.Mul.distribute).this.find(-Add).apply(Nat.Mul.distribute).this.find(-Add).apply(Nat.Mul.distribute)
