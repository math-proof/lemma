...
def apply(eq, Q_def, V_def, n=None):
    ...
    return Equal(Derivative[π](V_st_var._subs(s[t].var, s[0].var)),
                 Sum[s[1:t + 1].var, t:n](γ ** t * (Product[t:t](Pr(s[t + 1] | s[t])) * Sum[a[t].var](Derivative[π](Pr[a:π](a[t] | s[t])) * Q_st_var))) + \
                 γ ** n * Sum[s[1:n + 1].var](Product[t:n](Pr(s[t + 1] | s[t])) * Derivative[π](V_st_var._subs(s[t].var, s[n].var))))


@prove
def prove(Eq):
    from Lemma import Tensor, Algebra, Bool, Finset, Nat
