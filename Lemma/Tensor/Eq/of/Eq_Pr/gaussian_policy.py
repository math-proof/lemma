...
@apply
def apply(eq_given):
    (((a, a_var), (s, s_var)), [S[a], θ]), (exp, coeff) = eq_given.of(Equal[Pr[Conditioned[Equal[2]]], Exp * Expr])
...
@prove
def prove(Eq):
    from Lemma import Algebra, Calculus, Bool, Real
    ...
    a = a.var
    Eq << Bool.EqUFnS.of.Eq.apply(Eq[0], log)
