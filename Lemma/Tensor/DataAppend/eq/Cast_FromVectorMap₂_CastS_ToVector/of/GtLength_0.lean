...
-- imply
  ...
  have h_b_z : b_z[0] :: (b_z.tail ++ (m + n) :: s) = b_z ++ (m + n) :: s := by
    rw [List.Cons_Append.eq.AppendCons, List.EqCons_Tail.of.GtLength_0 (by grind)]
  A ++ B = cast (congrArg (Tensor α) h_b_z) (Tensor.fromVector (List.Vector.map₂ HAppend.hAppend A' B')) := by
-- proof
  intro h_tail h_head A' B' h_b_z
  simp [A', B']
  apply Eq_Cast.of.SEq.Eq h_b_z
  apply DataAppend.as.FromVectorMap₂_CastS_ToVector.of.GtLength_0 h
