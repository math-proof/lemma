...
-- imply
  ...
  S * A = (X * cast (congrArg (Tensor α) (by simp)) ((A.unsqueeze 0).repeat ⟨0, by grind⟩ n)).sum 0 := by
-- proof
  simp
  ...
  have h_s : (s.insertIdx 0 1).set 0 (n * (s.insertIdx 0 1)[0]) = n :: s := by
    simp
  have h_mul_stack := MulStack.eq.Stack_Mul.fin (cast (congrArg (Tensor α) h_s) ((A.unsqueeze 0).repeat ⟨0, by grind⟩ n)) (fun i => X[i])
