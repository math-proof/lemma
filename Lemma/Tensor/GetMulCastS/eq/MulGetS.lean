...
-- given
  ...
  (k : Fin l) :
-- imply
  (((cast (congrArg (Tensor α) (show ([] ++ [m, 1, l]).set 1 (n * ([] ++ [m, 1, l])[1]) = [] ++ [m, n, l] by grind)) ((A.unsqueeze 1).repeat n (1 : Fin 3)) * cast (congrArg (Tensor α) (show ([] ++ [1, n, l]).set 0 (m * ([] ++ [1, n, l])[0]) = [] ++ [m, n, l] by grind)) (((cast (congrArg (Tensor α) (show ([] ++ [l, n]).swap (([] ++ [l, n]).length - 2) (([] ++ [l, n]).length - 1) = [] ++ [n, l] by simp [EqSwap_0'1])) Bᵀ).unsqueeze 0).repeat m (0 : Fin 3))).get i).get j).get k =
