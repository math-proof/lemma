...
def extract_QVA(eq, Q_def, V_def=None):
    ...
    S[γ], (S[t], [S[t]]) = discount.of(Pow[Stack])
    ...
    if V_st_var:
        if weights:
            S[s[t].bvar], *S[weights] = V_st_var.of(Function)
        else:
            S[s[t].bvar], [S[γ]] = V_st_var.of(Function)
...
def apply(eq, Q_def, V_def):
    ...
    return Equal(V_st_var, Expectation(Q_st_var._subs(a[t].bvar, a[t]) | s[t], *limits_Q)),\
...
@prove
def prove(Eq):
    from Lemma import Tensor, Random, Bool
    ...
    Eq << apply(Equal(r[t] | s[:t] & a[:t], r[t]), # history-irrelevant conditional independence assumption for rewards based on states and actions
                Equal((Q ^ γ)(s[t].bvar, a[t].bvar), (1 - γ) * γ ** Stack[t](t) @ Expectation(r[t:] | s[t] & a[t])),
                Equal((V ^ γ)(s[t].bvar), (1 - γ) * γ ** Stack[t](t) @ Expectation(r[t:] | s[t])))
