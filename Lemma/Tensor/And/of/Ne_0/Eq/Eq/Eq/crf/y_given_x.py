from util import *


@apply
def apply(x_independence_assumption, y_independence_assumption, xy_independence_assumption, xy_nonzero_assumption):
    from Lemma.Tensor.Eq.of.Ne_0.Eq.Eq.Eq.crf.markov import process_assumptions
    x, y = process_assumptions(x_independence_assumption, y_independence_assumption, xy_independence_assumption, xy_nonzero_assumption)
    n, d = x.shape

    t = Symbol(domain=Range(n))
    i = Symbol(integer=True)

    joint_probability = Pr(x[:t + 1], y[:t + 1])
    emission_probability = Pr(x[i] | y[i])
    transition_probability = Pr(y[i] | y[i - 1])
    y_given_x_probability = Pr(y | x)
    y = pspace(y).symbol

    G = Symbol(Stack[y[i - 1], y[i]](log(transition_probability)))

    s = Symbol(Stack[t](log(joint_probability)))

    x = Symbol(Stack[y[i], i](log(emission_probability)))

    z = Symbol(Stack[y[t], t](Sum[y[:t]](E ** s[t])))

    x_quote = Symbol(Stack[t](log(z[t])))

    return Imply(t > 0, Equal(x_quote[t], Log(ReducedSum(Exp(x_quote[t - 1] + G))) + x[t])), \
        Equal(-log(y_given_x_probability), Log(ReducedSum(Exp(x_quote[n - 1]))) - s[n - 1])


@prove
def prove(Eq):
    from Lemma import Tensor, Set, Bool, Random, Real, Int, Finset, Nat

    from Lemma.Tensor.Eq.of.Ne_0.Eq.Eq.Eq.crf.markov import markov_assumptions
    d, n = Symbol(domain=Range(2, oo))
    x = Symbol(shape=(n, d), real=True, random=True)
    y = Symbol(shape=(n,), domain=Range(d), random=True)
    Eq << apply(*markov_assumptions(x, y))

    x_probability = Eq[3].lhs.arg.args[0]
    x = x_probability.lhs
    n = x.shape[0]
    s, t = Eq[4].lhs.args
    Eq.z_definition = Eq[5].apply(Tensor.Stack.of.All_Eq.fin, (Eq[5].lhs.indices[-1],))

    Eq << Tensor.Eq.of.Ne_0.Eq.Eq.Eq.crf.markov.apply(*Eq[:4])

    Eq << Tensor.Imp.of.Eq.crf.logits.apply(Eq[-1], Eq[8].lhs.base, Eq[7].lhs.base, s)

    Eq << Eq[-2].subs(t, t + 1)

    Eq << Eq[-1].this.args[1].apply(Int.NotInSub.of.NotIn_Icc, 1)

    Eq << Bool.ImpNot.of.Or.apply(Eq[-1])

    Eq << Eq.z_definition.subs(t, t + 1)

    Eq << Eq[-1].this.find(NotElement).apply(Int.NotInSub.of.NotIn_Icc, 1)

    Eq << Bool.ImpNot.of.Or.apply(Eq[-1])

    Eq <<= Eq[-1] & Eq[-4]

    Eq << Eq[-1].this.rhs.apply(Bool.Eq.of.Eq.Eq.subst.rhs)

    Eq << Eq[-1].this.find(Exp).apply(Real.ExpAdd.eq.MulExpS)

    Eq << Eq[-1].this.find(Sum).apply(Tensor.Sum.limits.pop.Slice)

    Eq << Eq[-1].this.find(Sum).apply(Finset.Sum.limits.separate)

    Eq << Eq[-1].this.find(Sum).apply(Tensor.Sum.eq.ReducedSum)

    Eq << Eq[-1].this.find(Stack).apply(Tensor.Stack.ReducedSum.eq.Dot)

    Eq.z_recursion = Eq[-1].subs(Eq.z_definition.reversed)

    Eq << Eq[6].subs(t, t + 1)

    Eq << Eq[-1].this.find(NotElement).apply(Int.NotInSub.of.NotIn_Icc, 1)

    Eq << Bool.ImpNot.of.Or.apply(Eq[-1])

    Eq <<= Eq.z_recursion & Eq[-1]

    Eq << Eq[-1].this.rhs.apply(Bool.Eq.of.Eq.Eq.subst.rhs)

    Eq << Eq[-1].this.find(Log).apply(Real.LogMul.eq.AddLogS.of.Ne_0.Ne_0)

    Eq.z_definition_by_x_quote = Bool.UFn.of.Eq.apply(Eq[6].reversed, exp)

    Eq << Eq[-1].subs(Eq.z_definition_by_x_quote)

    Eq << Eq[-1].this.find(MatMul).apply(Tensor.Dot.eq.Exp)

    Eq << Eq[-1].subs(t, t - 1)

    Eq << Eq[-1].this.find(NotElement).apply(Set.NotInAdd.of.NotIn, 1)

    Eq << Bool.ImpNot.of.Or.apply(Eq[-1])

    Eq << Eq[-1].this.lhs.apply(Nat.Ne.given.Gt)

    Eq.xy_joint_nonzero = Random.Ne_0.of.Ne_0.joint_slice.apply(Eq[3], (slice(0, t + 1), slice(0, t + 1)))

    Eq << Random.All_NeProb_0.All_NeProb_0.of.All_Ne0ProbJoint.apply(Eq.xy_joint_nonzero)

    y = Eq[-1].lhs.arg.lhs.base
    Eq << Random.All_Eq_MulProbCond.of.PSpace_Joint.apply(Eq[-2], y[:t + 1])

    Eq << Random.Sum.eq.Prob.apply(Sum[pspace(y[:t + 1]).symbol](Eq[-1].lhs))

    Eq << Eq[-2].subs(Eq[-1].reversed)

    Eq << Eq[-1].apply(Real.Or.Log.of.Eq)

    Eq << Bool.And_And.of.And.apply(Eq[-1] & Eq.xy_joint_nonzero)

    Eq << Eq[-1].this.rhs.apply(Real.LogMul.eq.AddLogS.of.Ne_0.Ne_0)

    Eq << Bool.UFn.of.Eq.apply(Eq[4].reversed, exp)

    Eq.y_given_x_log = Eq[-2].subs(Eq[-1])

    Eq << Eq.z_definition.apply(Tensor.Sum.of.Eq)

    Eq << Eq[-1].subs(Eq.z_definition_by_x_quote)

    Eq << Eq.y_given_x_log.subs(Eq[-1].reversed)

    Eq << Eq[-1].subs(t, n - 1)

    Eq << Eq[10].reversed + Eq[-1]

    # reference: Neural Architectures for Named Entity Recognition.pdf
    # https://spaces.ac.cn/archives/5542




if __name__ == '__main__':
    run()
# created on 2018-12-21
# updated on 2025-04-20
