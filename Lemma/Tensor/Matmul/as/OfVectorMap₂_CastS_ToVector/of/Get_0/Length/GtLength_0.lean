...
-- imply
  ...
  let Ys : List.Vector (Tensor α (s' ++ [n, k]).tail) s[0] := cast
    ...
    Y.toVector
  X.matmul Y (by grind) ≃ Tensor.OfVector (List.Vector.map₂ (fun X Y => Tensor.matmul (cast (congrArg (Tensor α) (show (s ++ [m, n]).tail = s.tail ++ [m, n] by grind)) X) (cast (congrArg (Tensor α) (show (s' ++ [n, k]).tail = (s'.tail ++ [n, k]) by grind)) Y) (by grind)) Xs Ys) := by
-- proof
  simp
  match s, s' with
  | [], [] =>
