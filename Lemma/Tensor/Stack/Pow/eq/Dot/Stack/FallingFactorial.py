...
def apply(self):
    ...
    return Equal(self, Stack[j:d, i:d](Stirling(i, j)) @ Stack[j:m, i:d](FallingFactorial(j, i)))
...
def prove(Eq):
    ...
    x = Symbol(real=True)
    Eq << Tensor.Stack.Mul.eq.Dot.Stack.FallingFactorial.apply(Stack[j:m, i:d](j ** i * x ** j))
