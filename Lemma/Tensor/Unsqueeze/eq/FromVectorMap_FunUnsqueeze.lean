...
-- proof
  ...
  have h_eq : Fin ((n :: s).tail.insertIdx d 1).prod = Fin (n :: s).tail.prod := by
    simp_all
  have := GetFlatten_AddMul.eq.Get.of.Lt.Lt i.isLt j.isLt ((⟨X⟩ : Tensor α (n :: s)).toVector.map (fun X ↦ List.Vector.map (fun k ↦ X.data[cast h_eq k]) (List.Vector.range (s.insertIdx d 1).prod)))
  simp_all
