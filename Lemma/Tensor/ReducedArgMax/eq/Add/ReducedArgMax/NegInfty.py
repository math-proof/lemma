...
def apply(self, k):
    ...
    return Equal(self, ReducedArgMax(BlockMatrix[len(x.shape) - 1](-oo * Ones(*x.shape[:-1], k), x)) - k)
...
def prove(Eq):
    ...
    Eq << Eq[0].this.find(ReducedArgMax[BlockMatrix]).apply(Tensor.ReducedArgMax.Block.eq.Add)
