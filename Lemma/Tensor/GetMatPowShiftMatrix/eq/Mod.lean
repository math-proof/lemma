...
-- proof
  ...
  | zero =>
    ...
    simp [Nat.Delta.eq.Ite, Fin.val_inj]
    rfl
  | succ k ih =>
    have hpow : (ShiftMatrix (α := α) e0 e1 ^ (((k : ℕ) + 1 : ℕ) : ℤ)) =
        (ShiftMatrix (α := α) e0 e1 ^ ((k : ℕ) : ℤ)) @ ShiftMatrix (α := α) e0 e1 := by
      rw [show (ShiftMatrix (α := α) e0 e1 ^ (((k : ℕ) + 1 : ℕ) : ℤ)) =
        Tensor.MatPow (ShiftMatrix (α := α) e0 e1) (((k : ℕ) + 1 : ℕ) : ℤ) from rfl]
      rw [show (ShiftMatrix (α := α) e0 e1 ^ ((k : ℕ) : ℤ)) =
        Tensor.MatPow (ShiftMatrix (α := α) e0 e1) ((k : ℕ) : ℤ) from rfl]
      ...
      exact MatProd.eq.DotMatProd (f := fun _ => ShiftMatrix (α := α) e0 e1)
    rw [hpow]
    ...
    ·
      ...
      exact (hmul_one _).trans (hsa.trans (by simp only [Nat.Delta.eq.Ite, hmod]; rfl))
    ·
      intro c _ hc
      have hiha := ih c
      rw [hiha]
      simp only [id, Nat.Delta.eq.Ite]
