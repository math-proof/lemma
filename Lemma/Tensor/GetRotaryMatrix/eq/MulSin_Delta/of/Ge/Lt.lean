...
-- given
  ...
  (hj : j < d) :
-- imply
  θ.rotaryMatrix[i][j] = θ.sin[j]'(by grind) * (KroneckerDelta (α := Fin d) ⟨i - d, by grind⟩ ⟨j, by grind⟩ : Tensor ℝ []) := by
-- proof
  unfold rotaryMatrix
  extract_lets I
  let C := I * [_ < d] θ.cos
  ...
  apply (GetHstack.eq.Get.of.Lt hj S C ⟨i - d, by grind⟩).trans
  apply (GetMulEye_Stack.eq.MulDelta θ.sin ⟨i - d, by grind⟩ ⟨j, by grind⟩).trans
