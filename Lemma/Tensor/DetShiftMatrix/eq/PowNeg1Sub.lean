...
-/
private def shiftPerm (n j₀ : ℕ) (hj : j₀ < n) (k : ℕ) (hk : j₀ + k < n) : Equiv.Perm (Fin n) :=
  match k with
  | 0 => 1
  | k + 1 =>
...
private lemma main
  ...
  (h : (j₀ : ℕ) < (i₀ : ℕ)) :
-- imply
  (ShiftMatrix (α := α) i₀ j₀).det = (-1) ^ ((i₀ : ℕ) - (j₀ : ℕ)) := by
-- proof
  have hi₀ : (i₀ : ℕ) < n := i₀.isLt
  have hj : (j₀ : ℕ) < n := j₀.isLt
  let k := (i₀ : ℕ) - (j₀ : ℕ)
  have hk : (j₀ : ℕ) + k < n := by omega
  let σ : Equiv.Perm (Fin n) := shiftPerm n (j₀ : ℕ) hj k hk
  have hσ1 : σ j₀ = i₀ := by
    have h := shiftPerm_j0 n (j₀ : ℕ) hj k hk
    have hki : (j₀ : ℕ) + k = (i₀ : ℕ) := by
      show (j₀ : ℕ) + ((i₀ : ℕ) - (j₀ : ℕ)) = (i₀ : ℕ)
      omega
    exact Fin.ext (congrArg (fun x : Fin n => (x : ℕ)) h |>.trans hki)
  have hσ2 : ∀ (x : Fin n), (j₀ : ℕ) < (x : ℕ) → (x : ℕ) ≤ (i₀ : ℕ) → σ x = ⟨(x : ℕ) - 1, by omega⟩ := by
    intro x hx1 hx2
    have htk1 : 1 ≤ (x : ℕ) - (j₀ : ℕ) := by omega
    have htk : (x : ℕ) - (j₀ : ℕ) ≤ k := by omega
    have hxb : (j₀ : ℕ) + ((x : ℕ) - (j₀ : ℕ)) < n := by omega
    have heq0 : (j₀ : ℕ) + ((x : ℕ) - (j₀ : ℕ)) = (x : ℕ) := by omega
    have heq : x = ⟨(j₀ : ℕ) + ((x : ℕ) - (j₀ : ℕ)), hxb⟩ := by
      apply Fin.ext
      exact heq0.symm
    rw [heq]
    exact_mod_cast shiftPerm_mid n (j₀ : ℕ) hj ((x : ℕ) - (j₀ : ℕ)) k htk1 htk hk
  have hσ3 : ∀ (x : Fin n), ((x : ℕ) < (j₀ : ℕ) ∨ (i₀ : ℕ) < (x : ℕ)) → σ x = x := by
    intro x hlt
    exact_mod_cast shiftPerm_out n (j₀ : ℕ) hj k hk (x : ℕ) x.isLt (by omega)
  have hT : (ShiftMatrix (α := α) i₀ j₀).toMatrix =
      (1 : Matrix (Fin n) (Fin n) (Tensor α [])).submatrix σ id := by
    ext i j
    have hentry := GetShiftMatrix.eq.Ite (α := α) i₀ j₀ i j
    ...
    if hi : (i : ℕ) = (j₀ : ℕ) then
      have hsi : σ i = i₀ := by
        have hi' : i = j₀ := Fin.ext hi
        rw [hi']
        exact hσ1
      simp only [hsi]
      split_ifs <;> first | rfl | (exfalso; omega) | erw [Nat.cast_one] | erw [Nat.cast_zero]
    else if hmid : (j₀ : ℕ) < (i : ℕ) ∧ (i : ℕ) ≤ (i₀ : ℕ) then
      simp only [hσ2 i hmid.1 hmid.2]
      split_ifs <;> first | rfl | (exfalso; omega) | erw [Nat.cast_one] | erw [Nat.cast_zero]
    else
      simp only [hσ3 i (by omega)]
      split_ifs <;> first | rfl | (exfalso; omega) | erw [Nat.cast_one] | erw [Nat.cast_zero]
  have hsign : ∀ m : ℕ, ∀ (hm : (j₀ : ℕ) + m < n),
      Equiv.Perm.sign (shiftPerm n (j₀ : ℕ) hj m hm) = (-1 : Units ℤ) ^ m := by
    intro m
    induction m with
    | zero =>
      intro _
    ...
    | succ m ih =>
      intro hmk1
      have hmk : (j₀ : ℕ) + m < n := by omega
      let a : Fin n := ⟨(j₀ : ℕ) + m, hmk⟩
      let b : Fin n := ⟨(j₀ : ℕ) + m + 1, hmk1⟩
      have hval : ((j₀ : ℕ) + m : ℕ) ≠ (j₀ : ℕ) + m + 1 := by omega
      have hne : a ≠ b := fun hcon => hval (congrArg Fin.val hcon)
      have hstep : shiftPerm n (j₀ : ℕ) hj (m + 1) hmk1 =
          Equiv.swap a b * shiftPerm n (j₀ : ℕ) hj m hmk := by
        rfl
      rw [hstep, Equiv.Perm.sign_mul, Equiv.Perm.sign_swap hne, ih hmk]
      rw [pow_succ']
  apply Eq.trans (Det.eq.DetToMatrix (ShiftMatrix (α := α) i₀ j₀))
