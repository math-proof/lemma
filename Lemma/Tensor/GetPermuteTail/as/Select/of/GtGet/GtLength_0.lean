...
import Lemma.Tensor.SEqSelectS.of.Eq.Val
...
-- proof
  if h_s : s.length = 1 then
    simp
    symm
    apply SEq.trans (SEqSelectS.of.Eq.Val (show ⟨s.length - 1, by grind⟩ = ⟨0, by grind⟩ by grind) (by rfl) (X := X) (i := ⟨k, by grind⟩) (i' := ⟨k, by grind⟩))
      ...
      ·
        ...
        ·
          ...
          ·
            ...
            have h_t := t.isLt
            simp at h_t
            rw [TailRotate.eq.Take.of.GtLength_0 (by omega)] at h_t
            simp
            ...
            ·
              ...
              have h_lt : k * (s.rotate (s.length - 1)).tail.prod + ↑t < (s.take (s.length - s.length)).prod * ((s.drop (s.length - s.length)).rotate (s.length ⊓ s.length - 1)).prod := by
                ...
                ·
                  rw [ProdDropLast.eq.DivProd.of.GtLength_0.Gt_0 (by grind) (by grind)]
                  rwa [EqDiv_Div.of.Ne_0.Dvd]
                  ·
                    apply Get.dvd.Prod.of.GtLength
                  ·
                    rw [Prod.eq.MulProdDropLast.of.GtLength_0 (by grind)]
              ...
              simp at h_q'
              have h_r' := r'.isLt
              simp [ProdRotate.eq.Prod] at h_r'
              ...
              ·
                have h_r' : r' < ((s.drop (s.length - s.length)).drop ((s.length ⊓ s.length - 1) % (s.drop (s.length - s.length)).length)).prod * ((s.drop (s.length - s.length)).take ((s.length ⊓ s.length - 1) % (s.drop (s.length - s.length)).length)).prod := by
                  simpa [MulProdS.eq.Prod.comm]
                let ⟨qₐ, rₐ, h_qₐrₐ⟩ := Any_Eq_AddMul.of.Lt_Mul h_r'
                let ⟨h_qₐ_div, h_rₐ_mod⟩ := Eq_Div.Eq_Mod.of.Eq_AddMul h_qₐrₐ
                rw [TailRotate.eq.DropLast.of.GtLength_0 (by grind)] at h_r'_div
                simp [ProdRotate.eq.Prod] at h_r'_div
                simp [Take_SubLength_1.eq.DropLast] at h_qₐ_div h_rₐ_mod
                ...
                rw [Prod.eq.MulProdDropLast.of.GtLength_0 (by grind) (s := s)]
                simp [DivMod_Mul.eq.ModDiv]
                rw [EqDivAddMul.of.Lt (by grind)]
