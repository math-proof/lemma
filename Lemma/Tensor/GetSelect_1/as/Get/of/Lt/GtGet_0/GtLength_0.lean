...
-- given
  ...
  (X : Tensor α (n :: s)) :
-- imply
  (X.select ⟨1, by grind⟩ ⟨i, by grind⟩).get ⟨j, by grind⟩ ≃ (X.get ⟨j, by simpa [Length.eq.Get_0.of.GtLength_0]⟩).get ⟨i, by rwa [LengthGet.eq.Get_0.of.GtGet_0.GtLength_1 (by grind) (by grind)]⟩ := by
-- proof
  ...
  ·
    ...
    ·
      ...
      erw [Select_1.eq.FromVectorMapToVector.of.GtLength_0 (by grind) (i := ⟨i, by grind⟩)]
