...
private lemma main
  ...
  {m n : ℕ} :
-- imply
  ((0 : Tensor α [m, n]).hstack (Tensor.eye m) ++ (Tensor.eye n).hstack (0 : Tensor α [n, m])).det = (-1) ^ (m * n) := calc
-- proof
  ...
  _ = ([i < m + n] [j < m + n]
    ...
    have hR' :=
      EqGetStack.fin
        ...
        j
    rw [show cast (congrArg (fun t => Tensor α [m + n, t]) (Nat.add_comm n m)) (blockSwap α m n) = cast (congrArg (Tensor α) hs) (blockSwap α m n) by congr 1]
    simp
    apply Eq.trans
      (Eq.of.SEq
  ...
  _ = ((1 : Matrix (Fin (m + n)) (Fin (m + n)) (Tensor α [])).submatrix id ((finRotate (m + n)) ^ m)).det := by
    ...
    have val_finRotate_pow (j : Fin (m + n)) : (((finRotate (m + n)) ^ m) j : ℕ) = if (j : ℕ) < n then m + (j : ℕ) else (j : ℕ) - n := by
      if hN : m + n = 0 then
        grind
      else
        have : NeZero (m + n) := ⟨hN⟩
        have val_finRotate_pow_mod {N k : ℕ} [NeZero N] (i : Fin N) : (((finRotate N) ^ k) i : ℕ) = (i.val + k) % N := by
          induction k with
          | zero =>
