...
-- proof
  if h_n_eq : s[s.length - 1] = s'[s'.length - 2] then
    have := Einsum.as.Tensordot.of.Get_SubLength.GeLength_2.GeLength_2 h_s h_s' h_n_eq X Y
    ...
    apply Einsum.as.Tensordot.of.GtGetS_SubLength.GeLength_2.GeLength_2 h_s h_s' (by omega) X Y
