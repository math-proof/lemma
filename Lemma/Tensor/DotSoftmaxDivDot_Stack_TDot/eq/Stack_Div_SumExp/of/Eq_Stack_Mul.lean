...
import Lemma.Tensor.DotDotSRotaryMatrix'.eq.Dot_DotRotaryMatrix'Sub
...
import Lemma.Tensor.RotaryMatrix'SubGetS.eq.Ite_RotaryMatrix'_T.of.Eq_Stack_Mul
  ...
  ·
    ...
    ·
      ...
      ·
        apply congrArg₂ <;> apply EqGetStack.fin
      ·
        apply Eq.trans (DotDotSRotaryMatrix'.eq.Dot_DotRotaryMatrix'Sub θ[i] θ[k] Q[i] K[k])
        apply congrArg (fun t : Tensor ℝ [d + d, d + d] => Q[i] @ (t @ K[k]))
        exact RotaryMatrix'SubGetS.eq.Ite_RotaryMatrix'_T.of.Eq_Stack_Mul (i := i) (j := k) hθ
  ·
    apply congrArg (fun t : Tensor ℝ [d + d] => id (α := Tensor ℝ []) (exp (t @ ([i < n] (R i) @ K[i])ᵀ / √↑(d + d))).sum)
    apply EqGetStack.fin
