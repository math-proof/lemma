...
-- proof
  ...
  | succ i ih =>
    ...
    apply EqCast.of.SEq
    rw [Stack.of.All_Eq.fin (f := fun t : Fin s[0] => (X[t]'(by apply GtLength.of.GtLength_0)).sum i) (g := fun t : Fin s[0] => ∑ k, (X[t]'(by apply GtLength.of.GtLength_0)).select ⟨i, h_lt_length_tail⟩ k)]
    ·
