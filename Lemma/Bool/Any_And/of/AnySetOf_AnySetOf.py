...
def prove(Eq):
    ...
    Eq << Set.Any_And_AndInS.of.Any_And.apply(Eq[-2], simplify=False)

    Eq << functions.Bool(Eq[-1].expr).this.arg.args[1].rhs.definition

    Eq << Eq[-1].this.rhs.arg.args[2].rhs.definition

    Eq << Bool.Iff.of.Bool.apply(Eq[-1])

    Eq << Bool.Cond.of.Cond.All_Iff.apply(Eq[-1], Eq[-4])
