...
def prove(Eq):
    ...
    Eq << Iff(All[i:m](Equal(functions.Bool(All[j:{a, b, c, d}](x[i, j] > 0)), 1)), All[j:{a, b, c, d}, i:m](x[i, j] > 0), plausible=True)

    Eq << Eq[-1].this.find(functions.Bool).apply(Bool.Bool.eq.Ite)

    Eq << Eq[-1].this.find(functions.Bool, All).apply(Bool.AllIn_Insert.Is.And_All)
