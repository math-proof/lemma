...
def prove(Eq):
    ...
    Eq << Iff(All[j:f(i), i:{a}](x[i, j] > 0), All[j:f(a)](x[a, j] > 0), plausible=True)

    Eq << Eq[-1].this.lhs.apply(Bool.All.doit.outer.setlimit)

    Eq << Iff(All[j:f(i), i:{b}](x[i, j] > 0), All[j:f(b)](x[b, j] > 0), plausible=True)
