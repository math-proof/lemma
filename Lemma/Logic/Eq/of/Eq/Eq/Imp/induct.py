...
def prove(Eq):
    ...
    h = Symbol(Stack[m](f[2 * m + 1] - g[2 * m + 1]))
    ...
    Eq << Logic.Eq_0.of.Eq_0.All_Imp.apply(Eq.is_zero, Eq.suffice, n=m)

    Eq << Eq[-1].this.lhs.definition

    Eq.odd = Eq[-1].reversed

    h = Symbol("h'", Stack[m](f[2 * m + 2] - g[2 * m + 2]))
    Eq << h[0].this.definition
    ...
    Eq << Logic.Eq_0.of.Eq_0.All_Imp.apply(Eq.is_zero_even, Eq.suffice_even, n=m)
