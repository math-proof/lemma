...
def prove(Eq):
    ...
    Eq << Eq[-1].this.rhs.expr.apply(Logic.Ite__Ite.eq.IteAnd_Not__Ite)

    Eq << Eq[-1].this.rhs().find(NotElement).simplify()

    Eq << Eq[-1].this.rhs.expr.apply(Logic.Ite.subst)
    ...
    Eq << Logic.UFn.of.UFn.Eq.apply(Eq[-1].reversed, Eq[-5])
    ...
    Eq << Logic.Ite__Ite.eq.IteAnd_Not__Ite.apply(Eq[-1].rhs.expr)
