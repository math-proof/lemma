...
def apply(given):
    ...
    d = Symbol(Stack[i:n](i) @ MatProd[i:n](SwapMatrix(n, i, b[i])))
    return All[p[:n]:P](Any[b[:n]](Equal(p[:n], Stack[k:n](a[d[k]]))))
...
def prove(Eq):
    ...
    Eq << Eq[-1].this.find(Stack).apply(Tensor.EqToMatrixStack)
