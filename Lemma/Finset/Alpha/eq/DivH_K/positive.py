...
def prove(Eq):
    ...
    Eq << Finset.Alpha.eq.Mul.HK.induct.apply(alpha(x[:_n + 1]))

    Eq << Bool.All.of.Cond.apply(Eq[-1], _n)

    Eq.n1 = Imply(Equal(n, 1), Eq.hypothesis, plausible=True)

    Eq << Eq.n1.this.apply(Bool.IffImpSAndEq)
    ...
    Eq.n1 = Bool.ImpOrS.of.Imp.Imp.apply(Eq.n1, Eq.n2)

    Eq.n0 = Imply(Equal(n, 0), Eq.hypothesis, plausible=True)

    Eq << Eq.n0.this.apply(Bool.IffImpSAndEq)
