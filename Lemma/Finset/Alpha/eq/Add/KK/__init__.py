...
def prove(Eq):
    ...
    Eq << Eq[0].this.lhs.apply(Finset.Alpha.eq.DivH_K.positive)

    Eq << Eq[-1].this.find(alpha).apply(Finset.Alpha.eq.DivH_K.positive)

    Eq << Eq[-1] - Eq[-1].rhs.args[0]

    Eq << Eq[-1].this.lhs.together()

    Eq << Finset.Add.eq.Pow.HK.recurrence.apply(x[:n + 1])
