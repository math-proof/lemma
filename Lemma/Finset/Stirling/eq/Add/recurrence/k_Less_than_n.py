...
def prove(Eq):
    ...
    Eq.Stirling2 = Eq[0].lhs.this.apply(Finset.Stirling.eq.Card)

    Eq.Stirling0 = Eq[0].rhs.args[1].this.apply(Finset.Stirling.eq.Card)

    Eq.Stirling1 = Eq[0].rhs.args[0].args[1].this.apply(Finset.Stirling.eq.Card)
    ...
    Eq << Finset.Abs.Imageset.Stirling.mapping.s0_B.apply(n, k, s0, B)

    Eq << Eq.s2_abs.subs(Eq[-1].reversed)

    Eq.A_union_abs = Eq.s2_abs_plausible.subs(Eq[-1]) - Card(s0)

    Eq << Finset.Abs.Cup.eq.Sum.Stirling.nonoverlapping.A.apply(n, k, A)
