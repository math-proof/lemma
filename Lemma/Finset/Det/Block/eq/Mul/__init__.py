...
@apply
def apply(self):
    from Lemma.Finset.DetBlock.eq.PowNegativeOneMul import swap_col
    X = self.of(Det)
...
def prove(Eq):
    ...
    Eq << (BlockMatrix([[Zeros(2 * n, n), Identity(2 * n), Zeros(2 * n, n)], [Identity(n), Zeros(n, 2 * n), Zeros(n, n)], [Zeros(n, n), Zeros(n, 2 * n), Identity(n)]]) @ Eq[0].lhs.arg).this.apply(Tensor.DotAppendSHstackS.eq.AppendHstackSAddSDotS, True)
    ...
    Eq << Eq[-1].this.rhs.apply(Tensor.DotAppendSHstackS.eq.AppendHstackSAddSDotS, True)

    X = Symbol(BlockMatrix([D.T, E.T]).T)
    Y = Symbol(BlockMatrix([B, G]))
    Z = Symbol(BlockMatrix([[A, C], [F, H]]))
    Eq << BlockMatrix([[X, Zeros(2 * n, 2 * n)], [Z, Y]]).this.find(Symbol).definition
