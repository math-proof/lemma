...
def prove(Eq):
    ...
    Eq << Finset.Det.eq.Sum.expansion_by_minors.apply(Eq.induct.lhs, i=n)
    ...
    Eq << Bool.And.given.And.trans.apply(Eq.deduct & Eq[-1])
    ...
    Eq << Finset.Det.eq.Sum.expansion_by_minors.apply(Det(Eq.column_transformation.rhs), i=i).this.rhs.simplify(deep=True)
    ...
    Eq << Bool.And.given.And.subst.All_Eq.apply(Eq.deduction & Eq[-1])
