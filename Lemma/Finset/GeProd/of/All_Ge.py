...
@prove
def prove(Eq):
    from Lemma import Bool, Set, Nat, Finset
    ...
    Eq << Eq.induct.this.find(Product).apply(Finset.Prod.eq.Mul.pop)

    Eq << Eq[-1].this.find(GreaterEqual[~Product]).apply(Finset.Prod.eq.Mul.pop)

    Eq << Bool.ImpAndS.of.Imp.apply(Eq[2], cond=f(n) >= g(n))

    Eq << Eq[-1].this.lhs.apply(Set.AllIn_Ico.Cond.given.AllIn_Icc.Le)

    Eq << Eq[-1].this.rhs.apply(Nat.GeMul.of.Ge.Ge)
