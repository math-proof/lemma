...
@prove
def prove(Eq):
    from Lemma import Finset, Algebra, Finset

    k = Symbol(integer=True)
    i, n = Symbol(integer=True, positive=True)
    Eq << apply(Sum[k:n](FallingFactorial(k, -i - 1)))

    Eq << Eq[0].this.find(FallingFactorial).apply(Finset.FallingFactorial.eq.Inv.FallingFactorial)
    ...
    Eq << FallingFactorial(k - 1, i).this.apply(Finset.FallingFactorial.eq.Mul.pop)

    Eq << FallingFactorial(k, i).this.apply(Finset.FallingFactorial.eq.Mul.shift)
    ...
    Eq << Eq[-1].this.rhs.apply(Finset.FallingFactorial.eq.Inv.FallingFactorial)
