...
def apply(n, Q=None):
    ...
    P_quote = Symbol("P'", conditionset(x[:n + 1], Equal(x[:n].cup_finset(), Range(n)) & Equal(x[n], n)))
...
def prove(Eq):
    ...
    Eq << Finset.SetOf.PQ_Equality.apply(n)

    Eq << Eq[2].subs(Eq[-1].reversed)

    u = Eq[-1].lhs.arg.indices[0]
    Eq << Finset.All_And.mapping.Qu2v.apply(n, n, u)

    Eq << Finset.All_And.mapping.Qu2v.apply(n, u, n)
