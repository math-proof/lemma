...
def prove(Eq):
    ...
    Eq << Eq[-1].apply(Set.Any_In.of.In_Cup)

    i = Eq[-1].variable
    _i = i.copy(domain=Range(n))
    Eq << Eq[-1].limits_subs(i, _i)

    Eq << Eq[1].lhs.this.apply(Set.Cup.eq.UnionCupS, cond={_i, n})

    Eq << Eq[-1].this.rhs.args[0].apply(Set.CupInsert.eq.Union_Cup, evaluate=False)

    Eq << Bool.Any_UFn.of.All_UFn.Any_Eq.apply(Eq[-3].reversed, Eq[-1])

    Eq.paradox = Eq[-1].subs(Eq[0])
    ...
    Eq << Bool.Any_UFn.of.All_UFn.Any_Eq.apply(Eq[-1].reversed, Eq[-3])
