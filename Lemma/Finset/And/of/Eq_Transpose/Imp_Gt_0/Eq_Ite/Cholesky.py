...
def apply(eq, infer, eq_piece):
    from Lemma.Finset.All.And.of.Eq_Transpose.Imp_Gt_0.Eq_Ite.Cholesky import extract
...
def prove(Eq):
    ...
    Eq << Finset.All.And.of.Eq_Transpose.Imp_Gt_0.Eq_Ite.Cholesky.apply(*Eq[:3], t)
    ...
    Eq << Eq.L_is_zero.this.rhs.apply(Set.In_Finset.of.Eq)
    ...
    Eq <<= Eq[-2].this.rhs.find(Less).apply(Nat.EqMin.of.Lt, reverse=True), Eq[-1].this.rhs.find(GreaterEqual).apply(Nat.EqMin.of.Ge, reverse=True)
