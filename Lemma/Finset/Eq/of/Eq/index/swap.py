...
def apply(given, i=None, j=None, w=None):
    ...
    if w is None:
        ...
        w = Symbol(Stack[_j, _i](SwapMatrix(n, _i, _j)))
...
def prove(Eq):
    ...
    Eq << Finset.And.of.Eq.index.apply(Eq[0], j=i)
    ...
    return
    Eq.union_equality, Eq.piecewise_equality = Set.Subset.then.eq.Union.apply(Eq[-2]), Eq.definition.subs(Eq[-1])
    ...
    Eq << Set.element.then.eq.piecewise.expr_swap.apply(Eq.dj_domain, Eq.piecewise_equality.lhs.args[2])
    Eq << Set.element.then.eq.piecewise.expr_swap.apply(Eq.di_domain, Eq.piecewise_equality.lhs.args[-1])
    Eq << Set.element.then.eq.intersection.apply(Eq.dj_domain)
    Eq << Eq[-2].subs(Eq[-1])
    Eq << Eq[-4] + Eq[-1]
    Eq << Eq.piecewise_equality.subs(Eq[-1])
    Eq << Finset.combinatorics.permutation.index.kronecker_delta.indexOf.apply(Eq[0], i, j)
    Eq << Eq[-1].subs(Eq.di_definition.reversed).subs(Eq.dj_definition.reversed)
