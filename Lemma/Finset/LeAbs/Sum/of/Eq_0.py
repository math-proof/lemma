...
def prove(Eq):
    ...
    Eq << Finset.Sum.eq.Neg.Sum.of.Eq_0.apply(Eq[0], Eq[1].find(Sum), i=i)

    Eq << Int.Abs.of.Eq.apply(Eq[-1])

    Eq << Finset.AbsSum.le.Sum_Abs.apply(Eq[-1].rhs.arg)

    Eq << Nat.Le.of.Eq.Le.apply(Eq[-2], Eq[-1])

    Eq << Eq[-1].this.find(Abs[Mul]).apply(Int.Abs.eq.Mul)

    Eq << Real.All_Le_Maxima.apply(Eq[1].find(Maxima))
