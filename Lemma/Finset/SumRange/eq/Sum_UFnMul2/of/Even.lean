...
import Lemma.Finset.SumSetOf_Even.eq.Sum_UFnMul2
...
-- given
  ...
  (f : ℤ → α) :
-- imply
  ∑ n ∈ (Range a b 2).toFinset, f n = ∑ n ∈ Finset.Ico (a / 2) ((b + 1) / 2), f (2 * n) := by
-- proof
  have h0 : a % 2 = 0 := Mod_2.eq.Zero.of.Even h
  ...
  have h_range1 : ∀ x : ℤ, x ∈ Range a b 1 ↔ a ≤ x ∧ x < b := by
    ...
    ·
      ...
      ·
        ...
        ring
  have h_fin : (Range a b 2).toFinset = {n ∈ Finset.Ico a b | n % 2 = 0} := by
    ext x
  ...
  rw [h_fin, SumSetOf_Even.eq.Sum_UFnMul2]
  have h_div : a / 2 = (a + 1) / 2 := by
    have ha : 2 * (a / 2) = a := by
      have := IntegerRing.div_add_mod a (2 : ℤ)
      simp [h0] at this
      omega
    have hne : (2 : ℤ) ≠ 0 := by decide
    rw [← ha]
    rw [Int.add_ediv_of_dvd_left (Int.dvd_mul_right 2 _)]
    simp [Int.mul_ediv_cancel_left _ hne]
  rw [h_div]
