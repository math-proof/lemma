...
def prove(Eq):
    ...
    Eq.variance = Eq[-1].this.rhs.args[0].apply(Finset.Sum.eq.Sub.unshift)

    Eq << Eq.variance.rhs.args[1].this.apply(Finset.Sum.limits.swap.intlimit)
    ...
    Eq << Eq[-1].this.rhs.args[0].apply(Finset.Sum.eq.Sub.push)
    ...
    Eq << Eq[-1].this.rhs.apply(Finset.Sum.eq.Sub.push)
    ...
    Eq.is_nonnegative = Nat.Le.given.Ge_0.apply(Eq[-1])

    x_ = Symbol('x', Stack[i](y[m - 1] - x[i]))
    ...
    Eq << Eq[-1].this.rhs.args[1].apply(Finset.Sum.limits.swap.intlimit)

    Eq << Eq[-1].this.rhs.args[1].doit()

    Eq << Eq[-1].this.rhs.args[0].limits_subs(i, j)

    Eq << Eq[-1].this.rhs.args[0].apply(Finset.Sum.eq.Sub.unshift)
