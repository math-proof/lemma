...
@prove
def prove(Eq):
    from Lemma import Finset, Random, Vector, Rat
    ...
    Eq << Random.Var.eq.Sum.of.Eq_Conditioned.apply(Eq[0], n)

    Eq << Eq[-1].this.find(ReducedSum).apply(Vector.Sum.eq.Sum_Get, k)

    Eq << Eq[-1].this.lhs.apply(Random.Var.Sum.eq.Add.Sum, j)

    Eq << Eq[-1].this.find(Covariance).apply(Random.Cov.eq.Sub.Expect)
