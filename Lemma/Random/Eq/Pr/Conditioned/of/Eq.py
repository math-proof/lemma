...
def prove(Eq):
    ...
    Eq << Eq[-1].lhs.this.apply(Random.Pr.eq.Div.Pr.bayes)

    Eq << Eq[-1].rhs.find(Pr).this.apply(Random.Pr.eq.Integral)
    ...
    Eq << Eq.plausible.rhs.find(Pr).this.apply(Random.Pr.eq.Integral)

    Eq << Eq[-1].this.rhs.find(Pr).apply(Random.Pr.eq.Integral.joint, x)
