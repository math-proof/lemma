...
@prove
def prove(Eq):
    from Lemma import Random, Real, Nat, Vector
    ...
    *Eq[-3:], Eq.hypothesis = apply(Equal(x[k] | x[:k], x[k]), Equal(Expectation(x[k]), μ), Equal(Variance(x[k]), σ ** 2), ε, n)
    ...
    Eq << Random.Le.Prob.Chebyshev_Inequality.of.Eq_Expect.Eq_Var.apply(Eq[-1], Eq.eq_var, sqrt(n) * ε / σ)

    Eq << Eq[-1].this.lhs.apply(Random.Prob.eq.Sub.SDiff)
    ...
    Eq << Eq[-1].this.find(Limit[Mul]).apply(Real.Lim.eq.Mul)
