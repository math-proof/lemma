...
def prove(Eq):
    ...
    Eq << apply(Expectation(Variance(y | x.random_argument)))

    Eq << Eq[0].this.find(Variance).apply(Random.Var.eq.Sub.Expect)

    Eq << Eq[-1].this.lhs.apply(Random.Expect.eq.Add)

    Eq << Eq[-1].this.find(Expectation[Mul]).apply(Random.Expect.eq.Mul)

    Eq << Eq[-1].this.lhs.apply(Random.Expect.law_of_total_expectation)
