...
def prove(Eq):
    ...
    Eq << Eq[-1].this.find(-(~Expectation) ** 2).apply(Random.Expect.importance_sampling, θ_quote)

    Eq << Eq[-1].this.rhs.find(Expectation[Mul]).apply(Random.Expect.importance_sampling, θ)
