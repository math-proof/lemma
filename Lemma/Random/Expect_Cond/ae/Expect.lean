...
private lemma map_yy_support_diag
  ...
  {π : MeasureTheory.Measure Ω} {y : Ω → γ}
-- given
...
-- proof
  ...
  exact (MeasureTheory.ae_map_iff hyym hp).2 (MeasureTheory.ae_of_all _ fun _ => by simp [JointRandomSymbol])
...
-- proof
  calc
    _ = ∫⁻ a, (φ a / c) * c ∂μ :=
      (MeasureTheory.lintegral_mul_const c (hφ.div measurable_const)).symm
    _ = ∫⁻ a, φ a ∂μ :=
      MeasureTheory.lintegral_congr fun _a ↦ ENNReal.div_mul_cancel hc0 hct
...
-- proof
  ...
  have hfin : ∀ᵐ b ∂ν, m b < ⊤ := by
    ...
    have htot : ∫⁻ b, m b ∂ν = 1 := by
      ...
      exact h.symm
    exact MeasureTheory.ae_lt_top hm (by rw [htot]; norm_num)
  have hcancel :
      ∫⁻ b, (∫⁻ a, f a b * (p (a, b) / m b) ∂μ) * u b ∂(ν.withDensity m) =
        ∫⁻ b, (∫⁻ a, f a b * p (a, b) ∂μ) * u b ∂ν := by
    rw [MeasureTheory.lintegral_withDensity_eq_lintegral_mul ν hm hMeas]
    -- Marginal: ∫ p(·,b) =ᵐ m
    have hmarg : (fun b ↦ ∫⁻ a, p (a, b) ∂μ) =ᵐ[ν] m := by
      have hsnd :
        ...
        rfl
      have hL :
          MeasureTheory.Measure.map Prod.snd ((μ.prod ν).withDensity p) =
        ...
        rw [MeasureTheory.Measure.map_apply measurable_snd hs,
          MeasureTheory.withDensity_apply p (measurable_snd hs),
          MeasureTheory.withDensity_apply (fun b ↦ ∫⁻ a, p (a, b) ∂μ) hs]
        have hind :
        ...
        have h1 :
          ...
          rw [← MeasureTheory.lintegral_indicator (measurable_snd hs) p]
        ...
        have h3 :
          ...
          have hpull :
            ...
            simpa using (MeasureTheory.lintegral_const_mul''
          ...
          rw [hind', MeasureTheory.lintegral_indicator hs]
      ...
      exact ((MeasureTheory.withDensity_eq_iff_of_sigmaFinite hm.aemeasurable hmInt.aemeasurable).1
        hμeq).symm
    ...
    · have hpint : ∫⁻ a, p (a, b) ∂μ = 0 := by rw [hmargeq, h0]
      ...
      refine Eq.symm ((MeasureTheory.lintegral_eq_zero_iff hφ).2 ?_)
      filter_upwards [hp0] with a ha
      simp [ha]
    · have : (∫⁻ a, f a b * (p (a, b) / m b) ∂μ) * m b =
          (∫⁻ a, (f a b * p (a, b)) / m b ∂μ) * m b := by
        refine congrArg (· * m b) (MeasureTheory.lintegral_congr fun a ↦ ?_)
  ...
  have hFub :
    ...
    have : ∫⁻ b, (∫⁻ a, f a b * p (a, b) ∂μ) * u b ∂ν =
      ...
      refine MeasureTheory.lintegral_congr fun b ↦ ?_
      exact (MeasureTheory.lintegral_mul_const (u b)
        ((hf.comp (measurable_id.prodMk measurable_const)).mul
          (hp.comp (measurable_id.prodMk measurable_const)))).symm
    rw [this]
    exact (MeasureTheory.lintegral_prod_symm (fun z : α × γ ↦ f z.1 z.2 * p z * u z.2)
  ...
  have hwd :
    ...
    have : ∫⁻ z, f z.1 z.2 * p z * u z.2 ∂(μ.prod ν) =
      ...
      ac_rfl
    rw [this]
    exact (MeasureTheory.lintegral_withDensity_eq_lintegral_mul (μ.prod ν) hp hobs).symm
  rw [hFub, hwd, ← hlaw]
  rw [MeasureTheory.lintegral_map' hobs.aemeasurable hxym]
...
-- proof
  ...
  have hunf :
    ...
    have hcond :
      ...
      dsimp [MeasureTheory.Measure.condProb, MeasureTheory.Measure.prob]
      rw [hden]
      simp [p', m', MeasureTheory.Measure.prob]
    rw [hcond]
  ...
  have hcancel :
    ...
    have hmarg : (fun z ↦ ∫⁻ a, p' (a, z) ∂μ) =ᵐ[νν] m' := by
      have hsnd :
        ...
        rfl
      have hL :
          MeasureTheory.Measure.map Prod.snd ((μ.prod νν).withDensity p') =
            νν.withDensity fun z ↦ ∫⁻ a, p' (a, z) ∂μ := by
        refine MeasureTheory.Measure.ext fun s hs => ?_
        rw [MeasureTheory.Measure.map_apply measurable_snd hs,
          MeasureTheory.withDensity_apply p' (measurable_snd hs),
          MeasureTheory.withDensity_apply (fun z ↦ ∫⁻ a, p' (a, z) ∂μ) hs]
        have hind :
    ...
    · have : (∫⁻ a, f a z.1 * (p' (a, z) / m' z) ∂μ) * m' z =
        ...
        refine congrArg (· * m' z) (MeasureTheory.lintegral_congr fun a ↦ ?_)
  ...
  have hFub :
    ...
    have : ∫⁻ z, (∫⁻ a, f a z.1 * p' (a, z) ∂μ) * u z.1 ∂νν =
      ...
      refine MeasureTheory.lintegral_congr fun z ↦ ?_
      exact (MeasureTheory.lintegral_mul_const (u z.1)
        ((hf.comp (measurable_id.prodMk measurable_const)).mul
          (hp'.comp (measurable_id.prodMk measurable_const)))).symm
    rw [this]
  ...
  have hwd :
    ...
    have : ∫⁻ w, f w.1 w.2.1 * p' w * u w.2.1 ∂(μ.prod νν) =
      ...
      ac_rfl
    rw [this]
    exact (MeasureTheory.lintegral_withDensity_eq_lintegral_mul (μ.prod νν) hp' hobs).symm
  rw [hFub, hwd, ← hlaw]
  rw [MeasureTheory.lintegral_map' hobs.aemeasurable hxyym]
  refine MeasureTheory.lintegral_congr fun ω ↦ ?_
  simp [JointRandomSymbol]
...
-- proof
  ...
  have hgg' : g =ᵐ[π.map y] g' := by
    ...
    have hmapg :
      ...
      have :
        ...
        (MeasureTheory.lintegral_map' (hg.mul hu).aemeasurable hym).symm
      rw [this]
      have hInd :
  ...
  rw [hunf, hunf']
  exact (MeasureTheory.ae_eq_comp hym hgg').symm
