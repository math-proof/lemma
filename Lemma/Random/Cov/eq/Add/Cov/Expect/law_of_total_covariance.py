...
def prove(Eq):
    ...
    Eq << Eq[0].this.rhs.find(Covariance).apply(Random.Cov.Expect.eq.Sub.Expect)

    Eq << Eq[-1].this.find(Expectation[Covariance]).apply(Random.Expect.Cov.eq.Sub.Expect)

    Eq << Eq[-1].this.lhs.apply(Random.Cov.eq.Sub.Expect)
