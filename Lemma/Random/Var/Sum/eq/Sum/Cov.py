...
@prove
def prove(Eq):
    from Lemma import Random, Bool, Finset
    ...
    Eq << Eq[-1].this.lhs.apply(Random.Var.Add.eq.Add.Cov)

    Eq << Eq[-1].this.find(Covariance).apply(Random.Cov.Sum.eq.Sum.Cov)

    Eq << Eq[-1].this.rhs.find(Sum).apply(Finset.SumIco.eq.AddSumIco.of.Le)

    Eq << Eq[-1].this.lhs.find(Sum[Covariance]).limits_subs(i, j)

    Eq << Eq[-1].this.find(Variance).apply(Random.Var.eq.Cov)
