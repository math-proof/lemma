...
@prove
def prove(Eq):
    from Lemma import Random, Real, Int

    x = Symbol(real=True, random=True)
    Eq << apply(Pr(x >= 0))

    Eq << Eq[0].this.lhs.apply(Random.Prob.eq.Integral)

    Eq << Eq[-1].this.rhs.find(Pr).apply(Random.Prob.eq.Integral)

    Eq << Int.Eq.given.Eq.transport.apply(Eq[-1], rhs=1)

    Eq << Eq[-1].this.lhs.apply(Real.Add.eq.Integral.concat)

    Eq << Eq[-1].this.lhs.apply(Random.Integral.eq.One)
