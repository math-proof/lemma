...
@apply
def apply(self):
    (((s, k), S[s[k].bvar]), (Z, S[s[:k].as_boolean()])), (S[k], S[0], n) = self.of(Product[Pr[Conditioned[Equal[Indexed], And]]])
    return Equal(self, Pr(s[:n] | Z), evaluate=False)


@prove
def prove(Eq):
    from Lemma import Random, Bool, Tensor, Finset
    ...
    Eq << Eq[-1].this.lhs.args[1].apply(Random.CondProb.eq.Div.CondProb, pivot=slice(1, None))
