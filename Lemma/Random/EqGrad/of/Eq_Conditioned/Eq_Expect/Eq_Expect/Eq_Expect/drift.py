...
def prove(Eq):
    ...
    Eq << Eq[-1].this.find(Derivative[Expectation]).apply(Random.Grad.Expect.eq.Expect.Grad)

    Eq << Eq[-1].this.find(Conditioned[~Derivative]).apply(Real.Grad.eq.Mul)

    Eq << Eq[-1].subs(π_quote, π)

    Eq << Eq[-1].this.rhs.find(Subs).apply(Random.Subs_Grad_KL.eq.Zero)

    Eq << Eq[-1].this.rhs.apply(Random.Expect.limits.desimp)
