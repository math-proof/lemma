...
def prove(Eq):
    ...
    Eq.y_nonzero, Eq.z_nonzero = Random.NePr_0.NePr_0.of.NePr__0.apply(Eq[-1])

    Eq.xy_probability = Random.Pr.eq.Mul.Pr.of.Ne_0.bayes.apply(Eq.y_nonzero, x)

    Eq << Random.Pr.eq.Mul.Pr.of.Ne_0.bayes.apply(Eq[2], x)

    Eq << Eq[-1].subs(Eq[0])

    Eq <<= Random.Integral.ae.Pr.of.Measurable.Measurable.Probability.apply(Integral[z.var](Eq[-1].lhs)), \
        Random.Integral.ae.Pr.of.Measurable.Measurable.Probability.apply(Integral[z.var](Eq[2].lhs)), \
