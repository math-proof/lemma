...
@prove
def prove(Eq):
    from Lemma import Random, Set, Bool, Nat, Finset, Rat, Int, Real
    ...
    *Eq[-6:], (Eq.ge_VF, Eq.ge_reward) = apply(Equal(r[t] | s[:t] & a[:t], r[t]), # history-irrelevant conditional independence assumption for rewards based on states and actions
    ...
    Eq << Eq[-1].this.lhs.apply(Random.Add.Expect.eq.Expect)
    ...
    Eq.MDV_pi_tilde, Eq.MDV_pi_hat, Eq.MDV_pi = Eq[-3].this.find(Expectation).apply(Random.Expect.eq.Sum_Mul_Prob), Eq[-2].this.find(Expectation).apply(Random.Expect.eq.Sum_Mul_Prob), Eq[-1].this.find(Expectation).apply(Random.Expect.eq.Sum_Mul_Prob)
    ...
    Eq << Eq[-1].this.find(Expectation).apply(Random.Expect.limits.desimp)

    Eq << Random.And.Ge.of.Eq_Conditioned.Eq_Expect.Eq_Expect.Eq_Expect.Ge.drift.apply(*Eq[:3], Eq[-1], Eq.ge_MDV)
