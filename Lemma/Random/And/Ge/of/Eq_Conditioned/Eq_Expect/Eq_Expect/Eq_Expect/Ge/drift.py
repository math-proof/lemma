...
def prove(Eq):
    ...
    *Eq[-5:], (Eq.ge_VF, Eq.ge_reward) = apply(Equal(r[t] | s[:t] & a[:t], r[t]), # history-irrelevant conditional independence assumption for rewards based on states and actions
    ...
    Eq << Eq[-1].this.find(Expectation).apply(Random.Expect.Conditioned.law_of_total_expectation)
    ...
    Eq << Random.Expect.ge.Inf.apply(Eq.ge.find(Expectation))
    ...
    Eq <<= Random.KL.ge.Zero.apply(Eq.ge.find(KL)), Random.Prob.ge.Zero.apply(Eq.ge.find(Pr)), Rat.Ne_0.of.Div1.gt.Zero.apply(Eq.ge)
    ...
    Eq <<= Eq[-2].this.find(MatMul).apply(Random.Dot.eq.Expect), Eq[-1].this.find(MatMul).apply(Random.Dot.eq.Expect)
    ...
    Eq << Random.GeExpect.of.Ge.apply(Eq[-1], (s[0],))
