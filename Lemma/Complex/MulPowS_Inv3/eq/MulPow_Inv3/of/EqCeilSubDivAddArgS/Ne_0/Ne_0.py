...
@prove
def prove(Eq):
    from Lemma import Bool, Real, Complex, Nat, Int
    ...
    Eq << Nat.Div.given.Eq.apply(Eq[-1], Mul(*Eq[-1].lhs.args[:2]))
    ...
    Eq << Eq[-1].this.rhs.args[1].arg.apply(Nat.Mul_Add.eq.AddMulS)

    Eq << Eq[-1].this.rhs.args[1].apply(Real.ExpAdd.eq.MulExpS)
