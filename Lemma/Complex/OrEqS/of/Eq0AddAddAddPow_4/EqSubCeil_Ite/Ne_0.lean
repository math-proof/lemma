import Lemma.Complex.Eq0AddAddAddPow_3.of.Eq_Ite_SubAdd_Pow_Inv3.EqSubCeil_Ite
...
-- proof
  ...
  let z : ℂ :=
    (if d = 0 then Ac + Bc else if d % 3 = 1 then Ac * ω + Bc else Ac * ~ω + Bc) - ar / 3
  have hcubic : z ^ 3 + ar * z ^ 2 + br * z + cr = 0 := by
    apply Eq0AddAddAddPow_3.of.Eq_Ite_SubAdd_Pow_Inv3.EqSubCeil_Ite (x := z) (a := ar) (b := br) (c := cr) (d := d)
    ·
      convert h₁
    ·
      extract_lets
