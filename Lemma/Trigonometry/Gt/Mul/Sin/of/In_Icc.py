...
@prove(proved=False)
def prove(Eq):
    from Lemma import Calculus, Algebra, Trigonometry, Nat
    ...
    # Eq << Eq[-1].this.rhs.args[2].apply(Nat.AddMulS.eq.Mul_Add)
    @Function(real=True)
    def h(x):
    ...
    Eq << Trigonometry.Mul_.Csc.Sin.eq.One.apply(x) * (2 * x)

    Eq << Trigonometry.Cos.eq.Mul_.Sin.Cot.apply(x) * x
