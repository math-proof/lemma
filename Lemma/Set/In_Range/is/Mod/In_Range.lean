...
private lemma mem_sliced_indices_pos
  ...
  ·
    ...
    ·
      obtain ⟨k, hk⟩ :=
        mem_sliced_indices_pos h_if h_stop h_step x h_tail
      refine ⟨k + 1, ?_⟩
      rw [hk]
...
private lemma mem_sliced_indices_pos_go
  ...
  | zero =>
    ...
    rw [List.mem_cons]
    have hk' : x.val = start := by simpa [zero_mul, add_zero] using hk
    exact Or.inl (Fin.ext hk')
  | succ k ih =>
    have hk_succ : x.val = start + (k + 1) * step := hk
    have hk' : x.val = (start + step) + k * step := by
      rw [hk_succ]
      ring
    have h_start_step : start + step < stop := by
      rw [hk_succ] at h_lt
      omega
    unfold Nat.sliced_indices
    rw [List.mem_cons]
    ...
    if h_if : start + step < stop then
      rw [dif_pos h_if]
      exact mem_sliced_indices_pos_go h_start_step h_stop h_step x k hk' h_lt
    else
      exfalso
      rw [hk_succ] at h_lt
      have h_ge : stop ≤ start + (k + 1) * step := by
        calc stop ≤ start + step := le_of_not_gt h_if
          _ ≤ start + (k + 1) * step := by
            gcongr
...
private lemma mem_sliced_indices_neg_go
  ...
  | zero =>
    unfold Nat.sliced_indices'
    rw [List.mem_cons]
    have hx_eq : x.val = start - 1 := by omega
    exact Or.inl (Fin.ext hx_eq)
  | succ k ih =>
    have hk_succ : x.val + (k + 1) * step + 1 = start := hk
    have h_step_le : step < start := by grind
    have hk' : x.val + k * step + 1 = start - step := by grind
    have h_start' : start - step ≤ n := by omega
    have h_start_step : start - step > stop := by
      have := hk_succ
      have := h_ge
      omega
    ...
    if h_if : start - step > stop then
      rw [dif_pos h_if]
      exact mem_sliced_indices_neg_go h_if h_start' h_step x k hk' h_ge
    else
      exfalso
      have h_le : start - step ≤ stop := le_of_not_gt h_if
      omega
  termination_by start - stop
  decreasing_by simp_wf; omega
...
private lemma mem_sliced_indices_neg
  ...
  ·
    ...
    ·
      have h_start' : start - step ≤ n := by omega
      have h_step_le : step ≤ start := by omega
      obtain ⟨k, hk⟩ :=
        mem_sliced_indices_neg h_if h_start' h_step x h_tail
      refine ⟨k + 1, ?_⟩
      calc
        x.val + (k + 1) * step + 1 = x.val + k * step + 1 + step := by ring
        _ = (start - step) + step := by rw [hk]
        _ = start := Nat.sub_add_cancel h_step_le
    ·
      simp at h_tail
...
private lemma mem_sliced_indices_neg_ge_stop
  ...
  ·
    ...
    ·
      have h_start' : start - step ≤ n := by omega
      exact mem_sliced_indices_neg_ge_stop h_if h_start' h_step x h_tail
    ·
      simp at h_tail
...
private lemma int_mod_eq_zero_of_add_mod_eq
  ...
  if hb0 : b = 0 then
    ...
    if hab_lt : a + b < d then
      have : (a + b) % d = a + b := Int.emod_eq_of_lt (by omega) hab_lt
      rw [this] at h'
      omega
    else
      have hab_ge : d ≤ a + b := by omega
      have hb' : b = d * ((a + b) / d) := by
        linarith [Int.emod_add_mul_ediv (a + b) d, h']
...
private lemma int_mod_eq_zero_of_sub_mod_eq
  ...
  (h : (m - j) % d = m % d) :
  j % d = 0 := by
  have hsum : (m % d + j % d) % d = m % d := by
    calc
      (m % d + j % d) % d = ((m - j) % d + j % d) % d := by rw [h]
      _ = ((m - j) + j) % d := by rw [← Int.add_emod]
      _ = m % d := by simp
  exact int_mod_eq_zero_of_add_mod_eq hd (by simpa [Int.add_emod, Int.emod_emod] using hsum)
...
private lemma nat_mod_eq_zero_of_add_mod_eq
  ...
  j % step = 0 := by
  have hj := int_mod_eq_zero_of_add_mod_eq (show (0 : ℤ) < (step : ℤ) from Nat.cast_pos.mpr h_step)
    (show ((start : ℤ) + j) % (step : ℤ) = (start : ℤ) % (step : ℤ) from by push_cast; exact_mod_cast h)
  exact_mod_cast hj
...
private lemma nat_mod_eq_zero_of_sub_mod_eq
  ...
  (h : (m - j) % step = m % step) :
  j % step = 0 := by
  have hj := int_mod_eq_zero_of_sub_mod_eq (show (0 : ℤ) < (step : ℤ) from Nat.cast_pos.mpr _h_step)
    (show ((m : ℤ) - j) % (step : ℤ) = (m : ℤ) % (step : ℤ) from by push_cast; exact_mod_cast h)
  exact_mod_cast hj
...
private lemma mem_sliced_indices_one
  ...
  if hx_eq : x.val = start then
    ...
    have hx_gt : start < x.val := Nat.lt_of_le_of_ne h_start_le (Ne.symm hx_eq)
    refine Or.inr ?_
    split_ifs with h_if
    ·
...
private lemma mem_sliced_indices_neg_one
  ...
  if hx_eq : x.val + 1 = start then
    have hx_val : x.val = start - 1 := by omega
    exact Or.inl (Fin.ext hx_val)
  else
    have hx_lt : x.val + 1 < start := by omega
    refine Or.inr ?_
    split_ifs with h_if
    ·
      have h_start' : start - 1 ≤ n := by omega
      exact mem_sliced_indices_neg_one h_if h_start' x (by omega) (by omega)
    ·
      exfalso
      omega
...
private lemma slice_range_step_one_mem
  {a b : ℤ}
  (h_start_lt :
    (Add_Mul_DivSub1Sign_2 n a).toNat < (Add_Mul_DivSub1Sign_2 n b).toNat.min n)
...
private lemma slice_range_step_succ_mem
  ...
  split_ifs
  · exact absurd ‹(Add_Mul_DivSub1Sign_2 n a).toNat ≥ (Add_Mul_DivSub1Sign_2 n b).toNat.min n›
      (not_le.mpr h_start_lt)
  · exact h_mem
...
private lemma slice_range_sign_neg_one_mem
  {a b : ℤ}
  (h_stop_lt :
    (Add_Mul_DivSub1Sign_2 n b + 1).toNat <
      (Add_Mul_DivSub1Sign_2 n a + 1).toNat.min n)
  ...
  split
  · rename_i step heq
    have : (0 : ℤ) ≤ Int.ofNat step := Int.natCast_nonneg step
    linarith [heq]
  · rename_i step heq
    split_ifs
    · exfalso
      exact absurd ‹(Add_Mul_DivSub1Sign_2 n a + 1).toNat.min n ≤ (Add_Mul_DivSub1Sign_2 n b + 1).toNat›
        (not_le.mpr h_stop_lt)
    · have hstep : step = 0 := Int.negSucc.inj heq.symm
...
private lemma main
  ...
  (x : Fin n) :
-- imply
  x ∈ Slice.range ⟨a, b, d⟩ n ↔
    x ∈ Slice.range ⟨a, b, sign d⟩ n ∧
      (x : ℤ) % d = (Slice.begin ⟨a, b, d⟩ n : ℤ) % d := by
-- proof
  constructor
  · intro h
    if hd₀ : d = 0 then
      ...
      | Int.ofNat (step + 1), _ =>
        ...
        ·
          ...
          obtain ⟨k, hk⟩ :=
            mem_sliced_indices_pos h_start_lt h_stop_le (Nat.succ_pos step) x h
          have h_x_lt_stop :=
            mem_sliced_indices_pos_lt_stop h_start_lt h_stop_le (Nat.succ_pos step) x h
          have h_start_le : start ≤ x.val := by
            rw [hk]
            omega
          constructor
          ·
            rw [hsign]
            exact slice_range_step_one_mem h_start_lt h_stop_le x h_start_le h_x_lt_stop
          ·
            ...
            have h_x : (x : ℤ) = (start + k * step.succ : ℤ) := by
              ...
              exact_mod_cast hk
            rw [h_x, h_norm]
            exact int_add_mul_emod (start : ℤ) (k : ℤ) (step + 1)
      | Int.negSucc _, hd =>
      ...
      | Int.negSucc step, _ =>
        ...
        ·
          ...
          have h_stop_lt :
              (Add_Mul_DivSub1Sign_2 n b + 1).toNat < start := by grind
          have h_start_le : start ≤ n := by grind
          obtain ⟨k, hk⟩ :=
            mem_sliced_indices_neg h_stop_lt h_start_le (Nat.succ_pos step) x h
          have h_stop_le :=
            mem_sliced_indices_neg_ge_stop h_stop_lt h_start_le (Nat.succ_pos step) x h
          have h_x_lt_start :=
            mem_sliced_indices_neg_lt_start h_stop_lt h_start_le (Nat.succ_pos step) x h
          ...
          ·
            ...
            rw [h_d]
            exact int_add_mul_emod (m : ℤ) (k : ℤ) (-[step+1])
      | Int.ofNat a, hd =>
        simp only [Slice.range] at h ⊢
        have : (0 : ℤ) ≤ a := by omega
      ...
      | Int.ofNat (step + 1), _ =>
        ...
        ·
          ...
          have h_mod_nat : x.val % step.succ = start % step.succ := by
            ...
            exact_mod_cast h1
          obtain ⟨j, hj⟩ :=
            mem_sliced_indices_pos h_start_lt h_stop_le Nat.one_pos x h_sign
          have h_x_lt_stop :=
            mem_sliced_indices_pos_lt_stop h_start_lt h_stop_le Nat.one_pos x h_sign
          have hj_mod : j % step.succ = 0 :=
            nat_mod_eq_zero_of_add_mod_eq (Nat.succ_pos step) (by simpa [hj, Nat.one_mul] using h_mod_nat)
          obtain ⟨k, hk⟩ := exists_eq_mul_of_mod_eq_zero (Nat.succ_pos step) hj_mod
          have h_mem :=
            mem_sliced_indices_pos_of_eq_add_mul h_start_lt h_stop_le (Nat.succ_pos step) x
              ⟨k, by rw [hj, hk, Nat.mul_comm, Nat.one_mul]⟩ h_x_lt_stop
      ...
      | Int.negSucc step, _ =>
        ...
        · rename_i step0 heq
          ...
          ·
            let start := (Add_Mul_DivSub1Sign_2 n a + 1).toNat.min n
            have h_stop_lt :
                (Add_Mul_DivSub1Sign_2 n b + 1).toNat < start := by
              dsimp [start]
            ...
            have h_norm : Slice.begin ⟨a, b, (-[step+1])⟩ n = m := by
              ...
              unfold Slice.begin
              simp
            have h_sign'' : x ∈ Nat.sliced_indices' h_stop_lt h_start_le Nat.one_pos := by
              simpa [hstep0] using h_sign'
            have h_stop_le :=
              mem_sliced_indices_neg_ge_stop h_stop_lt h_start_le Nat.one_pos x h_sign''
            obtain ⟨j, hj⟩ :=
              mem_sliced_indices_neg h_stop_lt h_start_le Nat.one_pos x h_sign''
            ...
            obtain ⟨k, hk⟩ := exists_eq_mul_of_mod_eq_zero (Nat.succ_pos step) hj_mod
            have h_mem :=
              mem_sliced_indices_neg_of_eq h_stop_lt h_start_le (Nat.succ_pos step) x
                ⟨k, by rw [← hk, ← hj]; omega⟩ h_stop_le
