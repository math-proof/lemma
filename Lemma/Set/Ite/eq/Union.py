...
def prove(Eq):
    ...
    Eq << Eq.suffice.this.find(Element).apply(Bool.OrAndS.of.BFn_Ite)

    Eq << Eq[-1].this.lhs.apply(Bool.Or.Is.Imp, index=slice(None, 2))

    Eq << Eq[-1].this.rhs.apply(Set.In_Union.given.OrInS, simplify=None)

    Eq << Eq[-1].this.rhs.find(Element).apply(Bool.BFn_Ite.given.OrAndS)

    Eq << Eq[-1].this.rhs.find(Element).apply(Bool.BFn_Ite.given.OrAndS)

    Eq << Eq[-1].this.rhs.apply(Bool.Or.Is.Imp, index=slice(None, 2))

    Eq << Eq.necessary.this.find(Element).apply(Set.OrInS.of.In_Union, simplify=None)

    Eq << Eq[-1].this.lhs.find(Element).apply(Bool.OrAndS.of.BFn_Ite)
