...
def prove(Eq):
    ...
    k = Eq[-1].rhs.variable
    x = Eq[-1].lhs.lhs
    Eq <<= Eq[-2].this.lhs.expr.apply(Set.Ge.of.In_Icc), Eq[-1].this.rhs.apply(Algebra.Any.given.Cond.subst, k, Floor(x))

    Eq <<= Eq[-2].this.lhs.apply(Logic.Any_And.of.AnySetOf_AnySetOf, simplify=None), Logic.Imp_And.given.Imp.Imp.apply(Eq[-1])

    Eq <<= Eq[-3].this.lhs.find(Element).apply(Set.Ge.of.In_Range), Logic.Imp.given.Cond.apply(Eq[-2]), Eq[-1].this.rhs.apply(Set.In_Range.given.And)

    Eq << Eq[-1].this.lhs.apply(Set.Ge.of.In_Icc)

    Eq <<= Eq[-3].this.lhs.expr.apply(Algebra.Ge.of.Ge.Ge), Set.In_Ico.given.Ge.Lt.apply(Eq[-2])
