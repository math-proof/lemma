...
def prove(Eq):
    ...
    Eq <<= Eq[-2].this.find(Element).apply(Set.In_Range.Is.And), Eq[-1].this.find(Element).apply(Set.In_Range.Is.And)
    ...
    Eq <<= Set.Or.given.NotIn.Range.apply(Eq[-2]), Set.Or.given.NotIn.Range.apply(Eq[-1])

    Eq <<= Set.NotIn.given.Eq_Empty.apply(Eq[-2]), Set.NotIn.given.Eq_Empty.apply(Eq[-1])

    Eq << Set.And.of.In_Ico.apply(Eq[0])

    Eq << Set.Eq_Empty.Range.of.Ge.apply(Eq[-2] + 1)

    Eq << Algebra.Le.of.Lt.strengthen.apply(Eq[-1])

    Eq << Set.Eq_Empty.Range.of.Le.apply(Eq[-1])
