...
-- proof
  have h' := h.symm
  have h_b := NotInfinite.of.NotInfinite.XEq h' h_a
  obtain h_eps | h_eps := OrInfinitesimalSSub.of.XEq h
  ·
    assumption
  ·
    if h_a_eps : a → 0 then
      apply InfinitesimalSub.of.Infinitesimal.Infinitesimal h_a_eps
      apply Infinitesimal.of.Infinitesimal.XEq h h_a_eps
    else
      have h_b_eps := NotInfinitesimal.of.NotInfinitesimal.XEq h' h_a_eps
