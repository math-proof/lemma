...
-- proof
  have h_or : ¬((b → ∞) ∧ y → ∞ ∧ (b + y = 0 ∨ ((b / (b + y)) → ∞) ∨ (y / (b + y)) → ∞)) := by
    intro ⟨h_b, h_y, h_bad⟩
    rcases h_bad with h_by | h_div_by | h_div_yb
    ·
      have h_st : stdPart (b / y) = -1 := by
        have hxy : b / y = -1 := by
          have hy : y = -b := by
            rw [add_comm] at h_by
            exact Eq_Neg.of.Add.eq.Zero h_by
          rw [hy, Div_Neg.eq.NegDiv]
          rw [Div.eq.One.of.Ne_0 (Ne_0.of.Infinite h_b)]
        simp [hxy]
      exact h_and ⟨h_b, h_st⟩
    ·
      have h_st : stdPart (b / y) = -1 := by
        by_cases h_eq : b = -y
        ·
          have hxy : b / y = -1 := by
            rw [h_eq, DivNeg.eq.NegDiv]
            rw [Div.eq.One.of.Ne_0 (Ne_0.of.Infinite h_y)]
          simp [hxy]
        ·
          exact EqStDiv.of.InfiniteDiv_Add.Ne_Neg.Infinite.left h_b h_eq h_div_by
      exact h_and ⟨h_b, h_st⟩
    ·
      have h_st : stdPart (b / y) = -1 := by
        by_cases h_eq : b = -y
        ·
          ...
          simp [hxy]
        ·
          exact EqStDiv.of.InfiniteDiv_Add.Ne_Neg.Infinite h_y h_eq h_div_yb
      exact h_and ⟨h_b, h_st⟩
  apply XEq.of.OrAndS
