...
def apply(n):
    ...
    return Equal(BlockMatrix([[SwapMatrix(n, i, j), Zeros(n)], [Zeros(n), S.One]]), SwapMatrix(n + 1, i, j))


@prove
def prove(Eq):
    from Lemma import Bool, Tensor, Int
    ...
    Eq << Eq[-1].apply(Tensor.EqStackS.of.Eq, (k,), (h,))
