...
def apply(is_nonzero, n, x1):
    ...
    return Equal(Det([Stack[j:n + 2](x1 ** j), Stack[j:n + 2](j * x1 ** j), Stack[j:n + 2, i:n](j ** i * x2 ** j)]), x1 * x2 ** Binomial(n, 2) * (x2 - x1) ** (2 * n) * Product[i:n](factorial(i)))
...
def prove(Eq):
    ...
    Eq << Eq[-1].this.find(Mul ** Mul).apply(Algebra.Pow.eq.Mul.split.base)
    ...
    Eq << (Eq[-1].lhs.arg @ Stack[j:n + 2, i:n + 2](Eq[2].lhs ** j * KroneckerDelta(i, j))).this.apply(Tensor.Dot.eq.Stack_Sum_MulGetS)
    ...
    Eq << Eq[-1].this.rhs.apply(Discrete.DetBlock_Stack_Pow.eq.MulProd_Factorial.vandermonde.n2)
