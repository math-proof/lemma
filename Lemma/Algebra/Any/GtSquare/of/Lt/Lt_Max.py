...
def prove(Eq):
    ...
    Eq <<= Logic.Cond.given.Imp.ImpNot.apply(Eq[-2], cond=m > 0), Logic.Cond.given.Imp.ImpNot.apply(Eq[-1], cond=m > 0)
    ...
    Eq <<= Eq[-5].this.rhs.rhs.apply(Logic.Cond.of.And, index=slice(2, None, -2), simplify=None), \
        ...
        Eq[-2].this.rhs.args[::2].apply(Logic.Cond.of.Eq.Cond.subst), \
    ...
    Eq <<= Eq[-2].this.rhs.rhs.args[:2].apply(Logic.Cond.of.Eq.Cond.subst), \
        Eq[-1].this.rhs.rhs.args[:2].apply(Logic.Cond.of.Eq.Cond.subst)
    ...
    Eq << Eq[-1].this.rhs.apply(Logic.Imp_And.of.Cond.Imp)

    Eq << Eq[-1].this.rhs.rhs.args[1:].apply(Algebra.Any.GtSquare.of.Gt_0.Lt)

    Eq << Eq[-1].this.rhs.rhs.apply(Logic.Cond.of.Eq.Cond.subst, reverse=True)
