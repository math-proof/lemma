...
def prove(Eq):
    ...
    Eq <<= Eq[-2].subs(Eq.eq_max), Eq[-3].this.expr.apply(Algebra.Any.given.Cond.subst, x, (m + y) / 2), Eq[-1].this.find(NotElement).apply(Set.NotIn_Icc.given.Or)
    ...
    Eq <<= Eq[-2].this.lhs.apply(Algebra.GtAdd.of.Gt.Gt), Logic.Imp.given.Cond.apply(Eq[-1])
