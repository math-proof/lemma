...
private lemma omega_ne_zero :
    -(1 / 2) + I * ↑(Real.sqrt 3) / 2 ≠ 0 := by
  rw [← eq_exp_omega]
  exact exp_ne_zero _


private lemma omega_conj_ne_zero :
    -(1 / 2) - I * ↑(Real.sqrt 3) / 2 ≠ 0 := by
  rw [← eq_exp_omega_conj]
  exact exp_ne_zero _
...
-- proof
  ...
  have harg_prod :
    ...
    rw [Arg.eq.Add.of.Ne_0.Ne_0 hUV_cbrt hfac, arg_cbrt hUV0, harg_fac]
    have hargUV := Arg.eq.Add.of.Ne_0.Ne_0 hU hV
    have hsum :
