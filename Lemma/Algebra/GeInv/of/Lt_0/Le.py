from util import *


@apply
def apply(is_positive, ge):
    a = is_positive.of(Expr < 0)
    x, a = ge.of(LessEqual)

    return GreaterEqual(1 / x, 1 / a)


@prove
def prove(Eq):
    from Lemma import Algebra, Bool, Int

    x, a = Symbol(real=True, given=True)
    Eq << apply(a < 0, x <= a)

    Eq << ~Eq[-1]

    Eq << Algebra.Ne.of.Lt.apply(Eq[0])

    Eq << Bool.Cond.of.Or_Not.Cond.apply(Eq[-1], Eq[-2])

    Eq.x_is_negative = Algebra.Lt.of.Lt.Le.apply(Eq[0], Eq[1])

    Eq << Algebra.Ne.of.Lt.apply(Eq.x_is_negative)

    Eq << Bool.Cond.of.Or_Not.Cond.apply(Eq[-1], Eq[-2])

    Eq << Int.Gt_0.of.Lt_0.Lt_0.apply(Eq[0], Eq.x_is_negative)

    Eq << ~Algebra.LtMul.of.Gt_0.Lt.apply(Eq[-1], Eq[-2]).reversed




if __name__ == '__main__':
    run()
# created on 2020-01-21
# updated on 2025-04-20
