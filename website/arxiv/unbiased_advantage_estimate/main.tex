...
Each item is a Lean module; the module path is the statement's name and a link to its interactive page.
\begin{enumerate}[leftmargin=1.6em,itemsep=0.35em]
  \item \textbf{Limits} (Section~\ref{sec:lim}). The library's textbook-style notation $\lim [n\to\infty]\,e=a$, which elaborates to mathlib's \texttt{Filter.Tendsto}; illustrated by \lmod{Real.Eq_0.Lim.of.LtAbs.IsFinite}~\cite{lemma_lim}.
  \item \textbf{Expectations and probabilities} (Section~\ref{sec:notation}). The library's $\E$/$\Prb$ binder notation and the colour convention of its renderer: red random variables, magenta random arguments, black observed values.
  \item \textbf{Model} (Section~\ref{sec:model}). A discounted MDP with finite $S$, $A$, a parametrised policy and the Ionescu-Tulcea trajectory measure; the value functions $V_t$, $Q_t$ as conditional expectations.
  \item \textbf{Bellman equations} (Section~\ref{sec:bellman}). \lmod{Tensor.And.Eq.Expect.of.Eq_Conditioned.Eq_Expect.Eq_Expect.Bellman}~\cite{lemma_bellman}.
  \item \textbf{Policy gradient by induction} (Section~\ref{sec:induct}). The recursion \lmod{Tensor.EqGrad.of.Eq_Conditioned.Eq_Expect.Eq_Expect.policy_gradient.recursion}~\cite{lemma_recursion}, its unrolling \lmod{Tensor.EqGrad.of.Eq_Conditioned.Eq_Expect.Eq_Expect.policy_gradient.induct}~\cite{lemma_induct}, the truncated identity \lmod{Tensor.Eq.Grad.Expect.of.Eq_Conditioned.Eq_Expect.Eq_Expect.policy_gradient}~\cite{lemma_pgn}, and its limit in the action-value form \lmod{Tensor.Eq.Dot.Grad.Expect.of.Eq_Conditioned.Eq_Expect.Eq_Expect.IsFinite.Q_Function}~\cite{lemma_pgq} and the REINFORCE form \lmod{Tensor.Eq.Dot.Grad.Expect.of.Eq_Conditioned.IsFinite.policy_gradient_theorem}~\cite{lemma_pgt}.
  \item \textbf{Main result} (Section~\ref{sec:main}). \lmod{Tensor.EqDot_GradExpect.of.Eq_Conditioned.Eq_Expect.IsFinite.IsFinite.unbiased_advantage_estimate}~\cite{lemma_uae}.
\end{enumerate}
...
On the formal side, CertRL~\cite{vajjha2021certrl} formalizes convergence of value and policy iteration in Coq; it does not treat stochastic policy gradients.
Our trajectory measure is mathlib's \texttt{Kernel.trajMeasure}~\cite{mathlib}, an implementation of the Ionescu-Tulcea theorem~\cite{kallenberg2002foundations}.
