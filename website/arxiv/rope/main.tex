...
\newcommand{\blk}{\mathrm{B}}
\newcommand{\blkcos}{\mathrm{B}_{\cos}}
\newcommand{\blksin}{\mathrm{B}_{\sin}}
...
We prove the following machine-checked results:
the realization
...
We machine-check the following groups of one-dimensional RoPE facts.
...
Its determinant is also formalized as
\(\det(\boldsymbol{P})=(-1)^{d/2}\)
(\lmod{Tensor.DetInterleave.eq.PowNeg1Div_2}),
which records the parity of the even/odd gather permutation.
...
The interleaved layout carries the same kernel with half-duplication replaced by pairwise duplication and \texttt{rotate\_half} by the pairwise swap-and-negate~$J$ (\lmod{Tensor.RotaryMatrix'.eq.AddMulSAppendRotaryMatrix'}).
...
\paragraph{Interleaved layout.}
All three identities carry over to~\(\rot'\).
\lmod{Tensor.RotaryMatrix'Neg.eq.TRotaryMatrix'} gives \(\rot'(-\alpha)=\rot'(\alpha)^\top\),
\lmod{Tensor.RotaryMatrix'0.eq.Eye} gives \(\rot'(0)=\eye_{d+d}\) (Lemma~\ref{lem:R0}),
and \lmod{Tensor.DotT_RotaryMatrix'.eq.Eye} gives
\(\rot'(\alpha)^\top @ \rot'(\alpha) = \eye_{d+d}\).
Each follows from the corresponding identity for~\(\rot\) by conjugation (Theorem~\ref{thm:conj}), or directly from the pairwise \(2\times 2\) structure of~\eqref{eq:rot-su}.
...
\begin{proposition}
\label{prop:eqdott}
\lmod{Tensor.DotT.eq.RotaryMatrixGet_Sub.of.Eq_Stack_Mul.Ge}.
...
The same pairing holds on the interleaved layout by conjugation (Theorem~\ref{thm:conj}):
\((\rot'(\alpha)\,q)^\top(\rot'(\beta)\,k)=q^\top\rot'(\beta-\alpha)\,k\)
(\lmod{Tensor.DotDotSRotaryMatrix'.eq.Dot_DotRotaryMatrix'Sub}).
...
The same reduction holds with $\rot'$ everywhere on the interleaved layout, by conjugation at every step (\lmod{Tensor.DotSoftmaxDivDot_Stack_TDot.eq.Stack_Div_SumExp.of.Eq_Stack_Mul}, \lmod{Tensor.RotaryMatrix'.eq.DotDot_RotaryMatrix}).
...
Every module in Table~\ref{tab:mods} builds with \texttt{lake build}.
...
\paragraph{Layout.}
Both pairings are formalized (\lmod{Tensor.RotaryMatrix.eq.AppendHstackSMulSEye}, \lmod{Tensor.RotaryMatrix'.eq.Stack_Ite_IteS}); the permutation lemma is \lmod{Tensor.RotaryMatrix'.eq.DotDot_RotaryMatrix}.
Kernel realization (\lmod{Tensor.DotRotaryMatrix.eq.AddMulS}) and pairing (\lmod{Tensor.DotDotSRotaryMatrix.eq.Dot_DotRotaryMatrixSub}) are stated for the Hugging Face layout; the interleaved counterparts (\lmod{Tensor.RotaryMatrix'.eq.AddMulSAppendRotaryMatrix'}, \lmod{Tensor.DotDotSRotaryMatrix'.eq.Dot_DotRotaryMatrix'Sub}, \lmod{Tensor.DotSoftmaxDivDot_Stack_TDot.eq.Stack_Div_SumExp.of.Eq_Stack_Mul}) follow by conjugation.
The matching matrix identity on the RoFormer layout is \lmod{Tensor.DotT_RotaryMatrix'.eq.RotaryMatrix'Sub}.
