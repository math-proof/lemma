...
export class LeanLogicAnd extends LeanLogic {
    ...
    jsonSerialize() {
        const lhs = this.lhs.jsonSerialize();
        const rhs = this.rhs.jsonSerialize();
        const f = this.func;
 ...
 * then `LeanTactic`, space-separated term, or a single `LeanToken` proof. Used for echo `:= by` serialization.
