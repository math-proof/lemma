...
const SMOKE = [
    ...
    { name: 'proof suffices', source: 'lemma s : True := by\n  suffices h : True from trivial\n  trivial\n' },
    { name: 'proof cases', source: 'lemma c : True := by\n  cases h : True with\n  | inl _ => trivial\n' },
    { name: 'proof rcases', source: 'lemma r : True := by\n  rcases h with ⟨⟩\n  trivial\n' },
    { name: 'proof obtain', source: 'lemma o : True := by\n  obtain ⟨⟩ := h\n  trivial\n' },
    { name: 'proof rintro', source: 'lemma ri : True := by\n  rintro ⟨⟩\n  trivial\n' },
    { name: 'proof intro', source: 'lemma i : True := by\n  intro h\n  trivial\n' },
    { name: 'proof revert', source: 'lemma rev : True := by\n  revert h\n  trivial\n' },
    { name: 'proof generalize', source: 'lemma g : True := by\n  generalize x : n = m\n  trivial\n' },
    { name: 'proof subst', source: 'lemma sub : True := by\n  subst h\n  trivial\n' },
    { name: 'proof contradiction', source: 'lemma co : False := by\n  contradiction\n' },
    { name: 'proof rfl', source: 'lemma rf : 1 = 1 := by\n  rfl\n' },
    { name: 'proof dsimp', source: 'lemma d : True := by\n  dsimp\n  trivial\n' },
    { name: 'proof change', source: 'lemma ch : True := by\n  change True\n  trivial\n' },
    { name: 'proof fail_if_success', source: 'lemma f : True := by\n  fail_if_success trivial\n  trivial\n' },
    { name: 'proof try', source: 'lemma t : True := by\n  try trivial\n  trivial\n' },
    { name: 'proof repeat', source: 'lemma rep : True := by\n  repeat trivial\n' },
    { name: 'proof conv', source: 'lemma cv : 1 + 1 = 2 := by\n  conv => lhs\n  rfl\n' },
    { name: 'attribute simp', source: 'attribute [simp] Nat.add_zero\n' },
    { name: 'notation', source: 'notation "|||" => Nat.add\n' },
    { name: 'local notation', source: 'local notation "~~" => Nat.mul\n' },
    { name: 'variable implicit braces', source: 'variable {α : Type*}\n' },
    { name: 'include', source: 'include h\n' },
    { name: 'omit', source: 'omit h\n' },
    { name: 'noncomputable def', source: 'noncomputable def nc : Nat := Classical.choice inferInstance\n' },
    { name: 'partial def', source: 'partial def pd : Nat → Nat\n  | 0 => 0\n  | n+1 => pd n\n' },
    { name: 'initialize', source: 'initialize foo : Nat\n' },
    { name: 'builtin_initialize', source: 'builtin_initialize bar : Nat\n' },
];
